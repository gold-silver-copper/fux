...
impl Ids {
    pub fn next_event_stream(&mut self) -> Option<u64> {
        self.next_event_stream = self.next_event_stream.checked_add(1)?;
        Some(self.next_event_stream)
    }
...
/// Opaque incarnation generated by the OS adapter at server startup, never by ECS systems.
#[derive(Resource, Clone, Debug)]
pub struct ServerInstance(pub String);
