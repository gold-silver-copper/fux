//! Terminal rendering primitives owned by fux. Adapted from koh; see LICENSES/koh.txt.
use std::io;
use vt100::Color;
mod termina;
...
/// The DEC private modes fux may have forwarded to the user's terminal (X10 `?9` + all mouse modes
/// and encodings, bracketed paste `?2004`, application cursor keys `?1`) plus normal keypad
/// (`ESC >`). Reset together whenever fux leaves the alternate screen — on drop *or* on suspend — so
...
/// is emitted by [`TerminalBackend::leave_alt_screen`], so every backend restores the same mode set.
...
///
/// Carries only what fux renders (fg/bg plus the boolean attributes vt100 exposes); `Copy` +
...
/// A pluggable terminal backend for the `fux connect` client.
///
...
/// a backend inside [`WorkspaceTerminal`](super::WorkspaceTerminal), which owns the mode ledger and the
...
mod tests {
    ...
    fn frame_and_screen_control_bytes() {
        assert_eq!(emit(TerminalBackend::begin_frame), b"\x1b[?2026h\x1b[?25l");
        assert_eq!(emit(TerminalBackend::end_frame), b"\x1b[?2026l");
        assert_eq!(
            emit(TerminalBackend::enter_alt_screen),
            b"\x1b[?1049h\x1b[?25l"
        );
    ...
    fn out_of_band_escapes() {
        ...
        assert_eq!(emit(TerminalBackend::bell), b"\x07");
    }
}
