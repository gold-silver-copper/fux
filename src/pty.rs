//! Pane PTY allocation, process-group ownership, resize and reaping.
//!
//! Adapted from koh 0.12.1 (MIT), Stephen Korzen. fux owns this pane lifecycle; koh
//! retains its separate standalone-shell adapter over the same portable-pty dependency.
//! The upstream copyright and license notice is retained in LICENSES/koh.txt.
//!
...
fn scrub_parent_env(cmd: &mut CommandBuilder) {
    scrub_parent_env_from(cmd, std::env::vars_os().map(|(key, _)| key));
}

fn scrub_parent_env_from(
...
pub(crate) fn is_private_env_key(key: &std::ffi::OsStr) -> bool {
    key.to_string_lossy().starts_with("KOH_") || key.to_string_lossy().starts_with("FUX_")
}

fn resolve_shell(shell_env: Option<std::ffi::OsString>) -> String {
    crate::config::default_shell_from(shell_env, None, cfg!(target_os = "android"))
}

/// Typed errors let callers distinguish PTY allocation, shell spawn, and resize failures.
///
...
// Own the child immediately: later master-handle or thread setup errors must not orphan it.
struct SpawnChildGuard(Option<Box<dyn portable_pty::Child + Send + Sync>>);
...
impl Pty {
    ...
    pub fn spawn(
        ...
        cmd.env("TERM", term);
        // Pane commands must not inherit application or credential environment settings.
        // Deliberate pane context is added by the host after this scrub.
        scrub_parent_env(&mut cmd);
        ...
        let mut child_guard = SpawnChildGuard(Some(child));
        ...
        Ok((
            Self {
                ...
                child: child_guard
                    .0
                    .take()
                    .ok_or_else(|| PtyError::Spawn(io::Error::other("missing spawned child")))?,
                killer,
...
mod tests {
    ...
    fn scrub_removes_koh_key_passphrase_even_when_inherited() {
        ...
        cmd.env("KOH_KEY_PASSPHRASE", "topsecret-unit");
        cmd.env("KOH_DNS", "1.1.1.1");
        cmd.env("FUX_SOCKET", "/old/parent/socket");
        assert!(
            cmd.get_env("KOH_KEY_PASSPHRASE").is_some(),
            "the builder seeds the parent env, so the var is present before scrubbing"
        );
        scrub_parent_env_from(
            &mut cmd,
            ["KOH_KEY_PASSPHRASE", "KOH_DNS", "FUX_SOCKET"].map(std::ffi::OsString::from),
        );
        assert!(cmd.get_env("FUX_SOCKET").is_none());
        assert!(
