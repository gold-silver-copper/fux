...
//! plus the terminal queries it answers on the host's behalf.
//! Adapted from koh under the MIT license; see LICENSES/koh.txt.

use crate::terminal::{MAX_TITLE_LEN, MAXIMUM_CLIPBOARD_SIZE, TerminalScreen, clamp_dims};
...
// Bound control strings independently of pane scrollback.
...
impl ServerTerminal {
    ...
    /// Capture the terminal state used to update a multiplexer pane.
...
mod tests {
    ...
    fn malformed_osc_9_4_yields_none_and_never_panics() {
        ...
        assert!(
            ring.first()
                .is_some_and(|payload| payload.len() <= UNHANDLED_OSC_MAX_LEN)
        );
    }
    ...
    fn unhandled_osc_ring_keeps_the_last_sixteen_and_drains() {
        ...
        assert_eq!(
            ring.first(),
            Some(&b"777;item4".to_vec()),
            "oldest retained is the 5th"
        );
        assert_eq!(ring.get(15), Some(&b"777;item19".to_vec()));
