...
use crate::machines::{
    MachineGuard,
    supervision::{ActionPhase, Freshness, PaneIdentity},
    transport::Transport,
};
use crate::paths::Paths;
use crate::remote::{
    client,
    descriptor::{self, Descriptor, DescriptorGuard},
};
...
pub fn control_descriptor(
    paths: &Paths,
    server: &str,
    machine: Option<&str>,
) -> Result<Descriptor, BevyError> {
    ...
    let (method, params) = match verb {
        ...
        MachineCmd::Add {
            ...
            for binding in attachments {
                let (workspace, path) = binding
                    .split_once('=')
                    .ok_or("attachment requires WORKSPACE=PATH")?;
                if bindings
                    .insert(workspace.to_owned(), direct(PathBuf::from(path))?)
                    .is_some()
                {
            ...
            (
                "zor/machine.add",
                json!({"name":name,"control":control,"attachments":bindings}),
            )
        }
        MachineCmd::Rename { machine, name } => {
            ("zor/machine.rename", json!({"machine":machine,"name":name}))
        }
        MachineCmd::Remove { machine } => ("zor/machine.remove", json!({"machine":machine})),
        MachineCmd::Control { machine, brp } => (
            "zor/machine.control",
            json!({"machine":machine,"control":brp.map(direct).transpose()?}),
        ),
        MachineCmd::Bind {
            machine,
            workspace,
            ...
            "zor/machine.bind",
            json!({"machine":machine,"workspace":workspace,"attachment":brp.map(direct).transpose()?}),
        ),
...
pub fn machine_task(
    ...
    let (method, task, operation, fux_instance) = match verb {
        super::TaskCmd::Stop { task } => (
            "zor/machine.stop",
            task,
            fux::attach::random_hex256()?,
            None,
        ),
        super::TaskCmd::Cancel { task } => (
            "zor/machine.cancel",
            task,
            fux::attach::random_hex256()?,
            None,
        ),
        super::TaskCmd::Resume {
            task,
            operation,
            fux_instance,
        } => (
    ...
    if snapshot.freshness != Freshness::Fresh {
        return Err(format!(
            "machine observation is {}; no action submitted",
    ...
    let view = snapshot
        .view
        .ok_or("machine has no observation; no action submitted")?;
    let mut tasks = view.rows.iter().filter(|row| row.id == *task);
    let row = tasks
        .next()
        .ok_or("task is not observed on this machine; no action submitted")?;
    ...
    let pane = if let Some(attempt) = row.current_attempt {
        let mut agents = view.agents.iter().filter(|agent| {
            agent.task.as_deref() == Some(task.as_str()) && agent.attempt == Some(attempt)
        });
        let agent = agents
            .next()
            .ok_or("current attempt has no observed pane; no action submitted")?;
    ...
    let mut params = serde_json::Map::from_iter([
        ("machine".into(), json!(snapshot.id)),
        ("task".into(), json!(task)),
        ("operation".into(), json!(operation)),
        ("guard".into(), json!(guard)),
    ]);
    if let Some(fux_instance) = fux_instance {
        params.insert("fux_instance".into(), json!(fux_instance));
    }
    ...
    let result = client::call_with(control, method, Value::Object(params))
        .map_err(|error| error.to_string())
        .and_then(|reply| {
            serde_json::from_value::<crate::remote::machine_methods::MachineActionResult>(reply)
                .map_err(|error| error.to_string())
        });
...
pub fn plugin(control: &Descriptor, verb: PluginCmd) -> Result<i32, BevyError> {
    ...
    let (method, params) = match verb {
        ...
        PluginCmd::Install { path, enabled } => (
            "zor/plugin.install",
            json!({"path":path.canonicalize()?.display().to_string(),"enabled":enabled}),
        ),
        PluginCmd::Link { path, enabled } => (
            "zor/plugin.link",
            json!({"path":path.canonicalize()?.display().to_string(),"enabled":enabled}),
        ),
        ...
        PluginCmd::Run {
            ...
            "zor/plugin.run",
            json!({"name":name,"action":action,"workspace":workspace,"task":task}),
        ),
        PluginCmd::Pane {
            name,
            pane,
            ...
            "zor/plugin.pane",
            json!({"name":name,"pane":pane,"workspace":workspace,"target":target,"task":task}),
        ),
        ...
        PluginCmd::Hook { .. } | PluginCmd::Supervise => {
            return Err("plugin helpers do not use controller CLI dispatch".into());
        }
    };
    ...
    if asynchronous {
        let operation = result
            ...
            .ok_or("plugin install omitted operation")?;
        let deadline = Instant::now() + Duration::from_secs(60);
        loop {
            result = client::call_with(
                control,
            ...
            match result.get("state").and_then(Value::as_str) {
                ...
                Some("pending") => return Err(format!(
                    "plugin operation {operation} is still pending; inspect it rather than resubmit"
                )
                .into()),
...
fn attachment_descriptor(
    ...
    let descriptor = match machine.filter(|name| !name.eq_ignore_ascii_case("local")) {
        Some(machine) => {
            let endpoint: crate::remote::machine_methods::MachineEndpoint =
                serde_json::from_value(client::call_with(
                    local,
                    "zor/machine.endpoint",
                    json!({"machine":machine, "workspace":pane.workspace}),
                )?)?;
            endpoint.descriptor
        }
        None => {
...
fn temporary_descriptor(
    paths: &Paths,
    descriptor: &Descriptor,
) -> Result<DescriptorGuard, BevyError> {
    paths.prepare()?;
    let path = paths
        .runtime_dir
        .join(format!("viewer-{}.brp.json", fux::attach::random_hex256()?));
...
pub fn attach(
    ...
    machine: Option<&str>,
    task: &str,
    expected: Option<&crate::dashboard::Target>,
) -> Result<i32, BevyError> {
    ...
    let mut live = record
        .attempts
        .iter()
        .filter(|attempt| attempt.state != crate::model::AttemptState::Finished);
    let attempt = live.next().ok_or("task has no live attempt")?;
    if live.next().is_some() {
        return Err("task has multiple live attempts".into());
    }
    if attempt.state != crate::model::AttemptState::Live
        || attempt.pane == 0
        || attempt.pid.is_none()
    {
    ...
    let pane = PaneIdentity {
        instance: attempt.instance.clone(),
        workspace: attempt.workspace.clone(),
        pane: attempt.pane,
        pid: attempt.pid,
    };
    if expected
        .is_some_and(|target| target.attempt != Some(id) || target.pane.as_ref() != Some(&pane))
    {
