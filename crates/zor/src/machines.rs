...
use bevy_asset::{
    Asset, AssetApp, AssetEvent, AssetEventSystems, AssetLoadFailedEvent, AssetLoader, AssetServer,
    Assets, Handle, LoadContext, io::Reader,
};
...
use supervision::{
    ActionFailure, ActionKind, ActionPhase, ActionRecord, ActionRequest, FRESH_MS, Freshness,
    MAX_ACTIONS, PaneIdentity, RemoteView, Report, Reports, Supervision, Worker,
};
...
impl AssetLoader for CatalogLoader {
    ...
    ) -> Result<CatalogAsset, CatalogError> {
...
impl Plugin for MachinesPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<MachinesFile>()
            .init_resource::<CatalogState>()
            .init_resource::<IntentLog>()
            .init_resource::<Reports>()
            .init_resource::<Wake>()
        ...
        if let Some(root) = file.asset_root
            && app.world().contains_resource::<AssetServer>()
        {
            app.init_asset::<CatalogAsset>()
                ...
                && let Some(name) = file.path.file_name()
            {
                let handle = app
                    ...
                    .load(name.to_string_lossy().into_owned());
                app.insert_resource(CatalogHandle(handle))
                    .add_systems(PostUpdate, asset_changes.after(AssetEventSystems));
            }
        }
    }
}
...
fn initialize(
    file: Res<MachinesFile>,
    mut state: ResMut<CatalogState>,
...
fn asset_changes(
    mut events: MessageReader<AssetEvent<CatalogAsset>>,
    mut failures: MessageReader<AssetLoadFailedEvent<CatalogAsset>>,
    handle: Res<CatalogHandle>,
    assets: Res<Assets<CatalogAsset>>,
    mut state: ResMut<CatalogState>,
    wake: Res<Wake>,
) {
    if events.read().any(
        |e| matches!(e, AssetEvent::Added{id} | AssetEvent::Modified{id} if *id == handle.0.id()),
    ) && let Some(asset) = assets.get(&handle.0)
    {
        ...
        if let Some(inbound) = &wake.0 {
            let _ = inbound.try_send(Inbound::Wake);
        }
    }
    for failure in failures.read().filter(|f| f.id == handle.0.id()) {
        state.problem = Some(failure.error.to_string());
    }
}
fn activate_catalog(
    mut commands: Commands,
    mut state: ResMut<CatalogState>,
    mut machines: Query<(Entity, &mut Binding, &mut MachineName, &mut Supervision)>,
    mut log: ResMut<IntentLog>,
) {
    let Some(catalog) = state.pending.take() else {
        return;
    };
    ...
    for (entity, mut binding, mut name, mut supervision) in &mut machines {
        ...
        if entry.control != binding.entry.control || entry.attachments != binding.entry.attachments
        {
            ...
            supervision.problem =
                Some("catalog endpoint changed; waiting for a fresh observation".into());
            binding.generation = generation;
        }
        if name.0 != entry.name {
            name.0.clone_from(&entry.name);
        }
        binding.entry = entry.clone();
    }
    for entry in &catalog.machines {
        if state.current.machines.iter().any(|old| old.id == entry.id) {
            continue;
        }
            ...
            .rev()
        {
            ...
            if record.phase == ActionPhase::Submitting {
        ...
        commands.spawn((
            Machine,
            MachineId(entry.id.clone()),
            MachineName(entry.name.clone()),
            Binding {
                entry: entry.clone(),
                generation,
            },
...
fn retire_actions(supervision: &mut Supervision, log: &mut IntentLog, state: &mut CatalogState) {
    for record in &mut supervision.actions {
        ...
        record.phase = ActionPhase::Uncertain;
        record.problem =
            Some("machine binding retired; outcome is unknown and is never replayed".into());
        if let Err(error) = log.complete(record) {
...
fn start_workers(
    mut commands: Commands,
    machines: Query<(Entity, &Binding), Without<Worker>>,
    reports: Res<Reports>,
    wake: Res<Wake>,
) {
    for (entity, binding) in &machines {
        if let Some(control) = &binding.entry.control {
            commands.entity(entity).insert(Worker::spawn(
                entity,
                binding.generation,
                control.clone(),
                reports.tx.clone(),
...
fn ingest_reports(
    reports: Res<Reports>,
    mut machines: Query<(&Binding, &mut Supervision)>,
    mut log: ResMut<IntentLog>,
    mut state: ResMut<CatalogState>,
) {
    for _ in 0..catalog::MAX_MACHINES * 2 {
        let Ok(report) = reports.rx.try_recv() else {
            break;
        };
        match report {
            Report::Poll {
                ...
                let Ok((binding, mut supervision)) = machines.get_mut(machine) else {
                    continue;
                };
                ...
                let outcome = outcome.and_then(|view| {
                    if binding
                        ...
                        .control
                        .as_ref()
                        .is_some_and(|t| t.descriptor().instance == view.instance)
                    {
                        Ok(view)
                    } else {
                        Err(supervision::Failure::Expired(
                            "service identity changed".into(),
                        ))
                    }
                });
            ...
            Report::Action {
                ...
                let Some(record) = supervision.actions.iter_mut().find(|a| a.id == id) else {
                    continue;
                };
                record.finished_ms = Some(at_ms);
                match outcome {
                    Ok(value) => {
                        record.phase = ActionPhase::Done;
                        record.result = Some(value);
                    }
                    Err(failure) => {
                        let (phase, problem) = match failure {
                            ActionFailure::Refused(p) => (ActionPhase::Refused, p),
                            ActionFailure::Failed(p) => (ActionPhase::Failed, p),
                            ActionFailure::Uncertain(p) => (ActionPhase::Uncertain, p),
                        };
                        record.phase = phase;
...
fn expire_views(clock: Res<Clock>, mut machines: Query<&mut Supervision>) {
    for mut supervision in &mut machines {
        supervision.expire(clock.now_ms);
    }
}
pub fn install_wake(world: &mut World, inbound: Sender<Inbound>) {
    world.resource_mut::<Wake>().0 = Some(inbound);
}
pub fn next_deadline(world: &mut World) -> Option<u64> {
    world
        ...
        .map(|s| s.observed_ms.saturating_add(FRESH_MS + 1))
        .min()
}
...
fn entity(world: &World, selector: &str) -> Result<Entity, String> {
    let entry = world
        ...
        .current
        .find(selector)
        .ok_or_else(|| format!("unknown machine {selector}"))?;
    world
        .resource::<Ids>()
        .machine(&entry.id)
        .ok_or_else(|| "machine configuration is being activated".into())
}
pub fn snapshot(world: &World, selector: &str) -> Result<MachineSnapshot, String> {
    let entity = entity(world, selector)?;
    let binding = world
        .get::<Binding>(entity)
        .ok_or("machine has no binding")?;
    let supervision = world
        .get::<Supervision>(entity)
        .ok_or("machine has no supervision")?;
    Ok(MachineSnapshot {
        id: binding.entry.id.clone(),
        name: binding.entry.name.clone(),
        freshness: supervision.freshness,
        observed_ms: supervision.observed_ms,
        view: supervision.view.clone(),
        problem: supervision.problem.clone(),
        actions: supervision.actions.iter().cloned().collect(),
    })
}
pub fn snapshots(world: &World) -> Vec<MachineSnapshot> {
    world
        .resource::<CatalogState>()
        .current
        .machines
        .iter()
        .filter_map(|m| snapshot(world, &m.id).ok())
        .collect()
}
pub fn catalog_problem(world: &World) -> Option<String> {
    world.resource::<CatalogState>().problem.clone()
}
pub fn reload(world: &mut World) -> Result<(), String> {
    let path = world.resource::<MachinesFile>().path.clone();
    match Catalog::load(&path) {
        Ok(catalog) => {
            let mut state = world.resource_mut::<CatalogState>();
        ...
        Err(error) => {
            ...
            world.resource_mut::<CatalogState>().problem = Some(text.clone());
            Err(text)
        }
    }
}
...
pub fn control_descriptor(world: &World, selector: &str) -> Result<Descriptor, String> {
    ...
    world
        .get::<Binding>(entity)
        .and_then(|b| b.entry.control.as_ref())
        .ok_or("machine has no control binding")?
        .resolve()
        .map(|r| r.descriptor)
        .map_err(|e| e.to_string())
}
...
pub fn endpoint_descriptor(
    ...
    let Some(workspace) = workspace else {
        return control_descriptor(world, selector);
    };
    let entity = entity(world, selector)?;
    let binding = world
        .get::<Binding>(entity)
    ...
    binding
        ...
        .attachments
        .get(workspace)
        .ok_or("workspace has no attachment binding")?
        .resolve()
        .map(|r| r.descriptor)
        .map_err(|e| e.to_string())
}
fn validate_guard(
    ...
    if let Some(pending) = &world.resource::<CatalogState>().pending {
        let next = pending
            .find(&binding.entry.id)
            .ok_or("machine removal is pending")?;
    ...
    let supervision = world
        .get::<Supervision>(entity)
        .ok_or("machine has no observation")?;
    let now = world.resource::<Clock>().now_ms;
    if supervision.freshness != Freshness::Fresh
        || now.saturating_sub(supervision.observed_ms) > FRESH_MS
    {
        return Err("machine observation is not fresh".into());
    }
    let view = supervision
        .view
        .as_ref()
        .ok_or("machine has no observation")?;
    if view.instance != guard.instance {
        return Err("remote service identity changed".into());
    }
    let row = view
        .rows
        .iter()
        .find(|t| t.id == task)
        .ok_or("task is not observed on this machine")?;
    if row.current_attempt != guard.attempt {
        return Err("task attempt changed".into());
    }
    if let Some(expected) = &guard.pane {
        let matches = view
            .agents
            .iter()
            .filter(|a| a.task.as_deref() == Some(task) && a.attempt == guard.attempt)
            .filter(|a| {
                a.instance == expected.instance
                    && a.workspace == expected.workspace
                    && a.pane == expected.pane
                    && a.pid == expected.pid
            })
            .count();
        if matches != 1 {
            return Err("exact pane identity changed or is ambiguous".into());
        }
    } else if guard.attempt.is_some() {
        return Err("active attempts require exact pane identity".into());
    }
    Ok(())
}
pub fn exact_attachment(
    world: &World,
    selector: &str,
    task: &str,
    guard: &MachineGuard,
) -> Result<ExactAttachment, String> {
    let entity = entity(world, selector)?;
    ...
    if descriptor.instance != pane.instance || descriptor.attach.is_none() {
        return Err("attachment binding does not identify the observed fux incarnation".into());
    }
    Ok(ExactAttachment {
        machine: binding.entry.id.clone(),
        task: task.into(),
        attempt: guard.attempt.ok_or("task has no attempt")?,
        pane,
        descriptor,
    })
}
pub fn queue_action(
    ...
    if !crate::model::valid_id(operation) {
        return Err("invalid operation id".into());
    }
    let entity = entity(world, selector)?;
    validate_guard(world, entity, task, &guard)?;
    if world.get::<Supervision>(entity).is_some_and(|s| {
        s.actions
            .iter()
            .filter(|a| a.phase == ActionPhase::Submitting)
            .count()
            >= MAX_ACTIONS
    }) {
        return Err("machine has too many unresolved actions".into());
    }
    if world.resource::<IntentLog>().find(operation).is_some() {
        return Err("operation intent is retained; inspect rather than replay".into());
    }
    if let ActionKind::Resume {
        operation: resume, ..
    } = &kind
        && resume != operation
    {
        return Err("resume operation differs from intent operation".into());
    }
    let actions = world
        .get::<Worker>(entity)
        .ok_or("machine worker is not available")?
        .actions
        .clone();
    if actions.is_full() {
        return Err("machine action queue is full".into());
    }
    let binding = world
        .get::<Binding>(entity)
        .ok_or("machine has no binding")?;
    ...
    let now = world.resource::<Clock>().now_ms;
    let id = {
        let mut reports = world.resource_mut::<Reports>();
        let id = reports.next_action;
        reports.next_action = id.checked_add(1).ok_or("action ids exhausted")?;
        id
    };
    ...
    world
        ...
        .map_err(|e| e.to_string())?;
    world
        .get_mut::<Supervision>(entity)
        .ok_or("machine disappeared")?
        .push_action(record.clone());
    if actions
...
mod tests {
    ...
    use super::*;
    use supervision::{Failure, RemoteAgent, RemoteTask};
    ...
    fn entry(id: &str, nonce: &str) -> MachineEntry {
        MachineEntry {
            ...
            control: Some(transport::Transport::Direct {
                host: "127.0.0.1".into(),
                port: 1234,
                brp: Descriptor {
                    instance: nonce.into(),
                    pid: 1,
                    http: crate::remote::Endpoint {
                        host: "127.0.0.1".into(),
                        port: 1234,
                    },
                    attach: None,
    ...
    fn app(entries: Vec<MachineEntry>) -> App {
        ...
        app.init_resource::<Ids>()
            ...
            .init_resource::<Reports>()
            .init_resource::<IntentLog>()
            .insert_resource(CatalogState {
                pending: Some(Catalog {
                    version: 1,
                    machines: entries,
                }),
                ..Default::default()
            ...
            .add_systems(
                PreUpdate,
                (activate_catalog, ingest_reports, expire_views).chain(),
            );
        app.update();
        app
    }
    ...
    fn stale_worker_and_service_identity_cannot_replace_observations() {
        ...
        app.update();
        assert_eq!(
            snapshot(app.world(), "one").unwrap().view,
            Some(first.clone())
        );
        assert_eq!(
            snapshot(app.world(), "two").unwrap().freshness,
            Freshness::Unauthorized
        );
        app.world_mut().resource_mut::<CatalogState>().pending = Some(Catalog {
            version: 1,
            machines: vec![entry("one", "replacement"), entry("two", "n2")],
        });
        poll(
            &app,
            "one",
        ...
        assert_eq!(
            snapshot(app.world(), "one").unwrap().freshness,
            Freshness::Offline
        );
        poll(
            &app,
    ...
    fn guarded_selection_refuses_changed_attempt_pane_and_stale_view() {
        ...
        wrong.attempt = Some(8);
        assert!(validate_guard(app.world(), entity, "task", &wrong).is_err());
        wrong = guard.clone();
        wrong.pane.as_mut().unwrap().pid = Some(43);
        assert!(validate_guard(app.world(), entity, "task", &wrong).is_err());
