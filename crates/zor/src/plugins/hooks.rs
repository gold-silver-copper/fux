...
use super::{
    HookCursors,
    manifest::{EventHook, pattern_matches},
};
...
pub fn run(name: &str) -> Result<(), String> {
    ...
    let hooks: Vec<EventHook> =
        serde_json::from_str(&std::env::var("ZOR_PLUGIN_HOOKS").map_err(|e| e.to_string())?)
            .map_err(|e| e.to_string())?;
    let cursors: HookCursors =
        serde_json::from_str(&std::env::var("ZOR_PLUGIN_CURSORS").map_err(|e| e.to_string())?)
            .map_err(|e| e.to_string())?;
    ...
    for (source, path, cursor, instance) in [
        ...
        (
            "fux",
            std::env::var_os("FUX_BRP").map(PathBuf::from),
            cursors.fux,
            cursors.fux_instance,
        ),
    ] {
        ...
        std::thread::Builder::new()
            .name(format!("plugin-hook-{source}"))
            .spawn(move || {
...
fn claim(
    ...
    let params = serde_json::Map::from_iter([
        ("name".into(), json!(name)),
        (source.into(), json!(cursor)),
        (format!("{source}_instance"), json!(instance)),
    ]);
    client::call_with(zor, "zor/plugin.cursor", Value::Object(params))
        .map(|_| ())
        .map_err(|e| e.to_string())
}
...
fn consume(
    ...
    loop {
        if instance != descriptor.instance {
            let _ = writeln!(
                std::io::stderr(),
                "zor: {source} hook stream incarnation changed; beginning at present"
            );
            ...
            let baseline = client::call_with(
                &descriptor,
                &format!("{source}/events.poll"),
                json!({"cursor": null}),
            )
            .map_err(|e| e.to_string())?;
            let activation = baseline
                ...
                .ok_or("invalid hook activation cursor")?;
        ...
        let result = client::stream(
            &descriptor,
            &format!("{source}/events+watch"),
            json!({"cursor": cursor}),
            |value| match handle_item(name, source, zor, &descriptor, &hooks, &mut cursor, value) {
                Ok(()) => {
        ...
        if let Err(e) = result {
            let _ = writeln!(
                std::io::stderr(),
                "zor: {source} hook stream disconnected: {e}"
            );
        }
...
fn handle_item(
    ...
    let value = if value.get("jsonrpc").is_some() {
        client::unwrap_reply(value).map_err(|e| e.to_string())?
    } else {
        value
    };
    if let Some(gap) = value.get("gap").filter(|g| !g.is_null()) {
        let resume = gap
            .get("resume")
            .and_then(Value::as_u64)
            .ok_or("invalid hook gap")?;
        let _ = writeln!(
            std::io::stderr(),
            "zor: {source} hook event gap {gap}; missing events are not replayable"
        );
    ...
    let events = value
        ...
        .ok_or("invalid hook stream item")?;
    for event in events {
        let next = event
            .get("cursor")
            .and_then(Value::as_u64)
        ...
        let event_name = event
            ...
            .ok_or("invalid event name")?;
        let qualified = if event_name.starts_with(&format!("{source}/")) {
            event_name.to_owned()
        } else {
            format!("{source}/{event_name}")
        };
        ...
        *cursor = next;
        for hook in hooks
            .iter()
            .filter(|h| pattern_matches(&h.pattern, &qualified))
        {
            let Some((program, args)) = hook.command.split_first() else {
                return Err("empty hook command".into());
            };
            let payload = serde_json::to_string(event.get("event").unwrap_or(&Value::Null))
                .map_err(|e| e.to_string())?;
            let mut command = Command::new(program);
            command
                .args(args)
                .stdin(Stdio::null())
                .env("ZOR_PLUGIN_KIND", "event")
                .env("ZOR_PLUGIN_EVENT_NAME", &qualified)
                .env("ZOR_PLUGIN_EVENT", &payload)
                .env("ZOR_PLUGIN_EVENT_CURSOR", next.to_string())
                .env("ZOR_PLUGIN_EVENT_INSTANCE", &descriptor.instance);
            let status = command
                .status()
                .map_err(|e| format!("hook {qualified}: {e}"))?;
            if !status.success() {
                let _ = writeln!(
                    std::io::stderr(),
                    "zor: hook {qualified} exited {status}; claim retained, not retried"
                );
            }
        }
    }
