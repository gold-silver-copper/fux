...
described!(
    pub struct DashboardRowsParams {
        #[serde(default)]
...
described!(
    pub struct DashboardInputParams {
        ...
        pub revision: u64,
        #[serde(default)]
        pub provider_node: Option<u64>,
        pub input: Input,
    }
);
described!(
    pub struct DashboardReturnParams {
        pub id: u64,
        pub handoff: u64,
    }
);
...
fn rows(mut req: Request, world: &mut World) -> BrpResult {
    ...
    to_value(DashboardRows {
        rows: dashboard::rows(world, params.machine.as_deref()).map_err(invalid)?,
    })
}
