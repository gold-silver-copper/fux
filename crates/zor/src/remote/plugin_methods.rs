...
use super::methods::{
    MethodSpec, NoParams, Request, codes, described, error, handler, spec, to_value, unauthorized,
};
use super::token::Capabilities;
use crate::plugins::{self, Binding, HookCursors, PluginError, PluginRecord, RunContext};
...
described!(
    pub struct PluginRunParams {
        ...
        pub expected_fux_instance: Option<String>,
    }
);
described!(
    pub struct PluginPaneParams {
        pub name: String,
        pub pane: String,
...
described!(
    pub struct PluginCursorParams {
        pub name: String,
        pub zor: Option<u64>,
        pub fux: Option<u64>,
        pub zor_instance: Option<String>,
        pub fux_instance: Option<String>,
    }
);
described!(
...
described!(
    pub struct PluginSubmitted {
        pub operation: u64,
    }
);
described!(
    pub struct PluginOperationParams {
        pub operation: u64,
    }
);
described!(
...
fn admin(req: &Request, world: &World) -> Result<(), BrpError> {
...
fn inspect(mut req: Request, world: &mut World) -> BrpResult {
    ...
    to_value(PluginInspected {
        plugin: plugins::record(world, plugin),
    })
}
fn install(mut req: Request, world: &mut World) -> BrpResult {
    admin(&req, world)?;
    let p: PluginPathParams = req.parse()?;
    to_value(PluginSubmitted {
        operation: plugins::install(world, std::path::Path::new(&p.path), p.enabled)
            .map_err(failure)?,
    })
}
fn link(mut req: Request, world: &mut World) -> BrpResult {
    ...
    let p: PluginPathParams = req.parse()?;
    to_value(PluginSubmitted {
        operation: plugins::link(world, std::path::Path::new(&p.path), p.enabled)
            .map_err(failure)?,
    })
}
...
fn run(mut req: Request, world: &mut World) -> BrpResult {
        ...
        && world.resource::<crate::lifecycle::Link>().instance.as_ref() != Some(expected)
    {
        return Err(error(
            codes::INVALID,
            "plugin action belongs to a different fux incarnation",
        ));
    }
    let plugin = plugins::find(world, &p.name).map_err(failure)?;
    to_value(PluginRunStarted {
        run: plugins::run_action(
            world,
            plugin,
            &p.action,
            RunContext {
...
fn pane(mut req: Request, world: &mut World) -> BrpResult {
    ...
    let plugin = plugins::find(world, &p.name).map_err(failure)?;
    to_value(PluginPaneOpened {
        opening: plugins::open_pane(world, plugin, &p.pane, &p.workspace, p.target, p.task)
            .map_err(failure)?,
    })
}
fn open_link(mut req: Request, world: &mut World) -> BrpResult {
    ...
    to_value(PluginLinkOpened {
        name: world
            .get::<crate::model::PluginId>(plugin)
            .map(|id| id.0.clone())
            .unwrap_or_default(),
        run,
    })
...
fn logs(mut req: Request, world: &mut World) -> BrpResult {
    ...
    let plugin = plugins::find(world, &p.name).map_err(failure)?;
    to_value(PluginLogs {
        lines: plugins::logs(world, plugin, p.limit).map_err(failure)?,
    })
}
fn bindings(mut req: Request, world: &mut World) -> BrpResult {
    let NoParams {} = req.parse()?;
    to_value(PluginBindings {
        bindings: plugins::bindings(world),
    })
}
fn cursor(mut req: Request, world: &mut World) -> BrpResult {
    ...
    to_value(PluginCursor {
        cursors: plugins::set_cursor_for(
            world,
            plugin,
            p.zor,
            p.fux,
...
handler!(brp_install, install);
handler!(brp_link, link);
handler!(brp_enable, enable);
handler!(brp_disable, disable);
handler!(brp_run, run);
handler!(brp_pane, pane);
handler!(brp_open_link, open_link);
handler!(brp_logs, logs);
handler!(brp_bindings, bindings);
handler!(brp_cursor, cursor);
