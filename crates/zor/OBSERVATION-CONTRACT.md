...
exit status, and child cleanup. The `zor <command>` passthrough integration suite (feature `wrap`) covers these contracts.
...
Unmatched screens evaluate to `unknown`, never implicit idle. Both `zor <command>` and the single-pane
