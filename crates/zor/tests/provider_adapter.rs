...
fn adapter() -> (
    ProviderAdapter,
    async_channel::Receiver<Inbound>,
    ...
    (
        ProviderAdapter::with_executable(tx, env!("CARGO_BIN_EXE_zor").into()),
        rx,
        attempt,
    )
}
...
fn until(mut predicate: impl FnMut() -> bool) {
    ...
    while !predicate() {
        assert!(
            start.elapsed() < LIMIT,
            "provider process failed to settle boundedly"
        );
...
fn started(
    ...
    attempt: bevy_ecs::entity::Entity,
) -> (Group, Pid) {
    let mut leader = None;
...
fn close_reaps_group_even_when_eof_is_ignored_and_stdout_is_inherited() {
    ...
    assert!(
        adapter
            .apply(spawn(attempt, RETAIN_STDOUT))
            .unwrap()
            .is_none()
    );
...
fn blocked_stdin_rejects_overflow_and_shutdown_still_finishes() {
    ...
    assert!(
        adapter
            .apply(spawn(attempt, BLOCK_STDIN))
            .unwrap()
            .is_none()
    );
    ...
    assert!(
        rejected,
        "a blocked child must not accept an unbounded stdin queue"
    );
    adapter.shutdown();
...
fn immediate_drop_cannot_leave_a_queued_spawn_alive() {
    ...
    assert!(
        adapter
            .apply(Effect::SpawnProvider {
                attempt,
                argv: vec![
                    "/bin/sh".into(),
                    "-c".into(),
                    "printf '%s' \"$$\" > \"$MARKER\"; exec sleep 60".into()
                ],
                cwd: None,
...
fn ordinary_stdout_precedes_exit_and_preserves_attempt() {
    ...
    assert!(
        adapter
            .apply(spawn(attempt, "printf 'evidence-frame\\n'"))
            .unwrap()
            .is_none()
    );
    let mut began = false;
...
fn host_hard_death_does_not_leave_provider_descendants_running() {
    ...
    let mut host = Host(
        Command::new(std::env::current_exe().unwrap())
            ...
            .env("ZOR_PROVIDER_HOST_FIXTURE", &marker)
            .spawn()
            .unwrap(),
    );
    let mut pids = None;
...
fn provider_host_fixture() {
    let Some(marker) = std::env::var_os("ZOR_PROVIDER_HOST_FIXTURE") else {
        return;
    };
    let (mut adapter, rx, attempt) = adapter();
    assert!(
