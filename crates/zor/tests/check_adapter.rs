...
fn spawn(check: Entity, cwd: &std::path::Path, script: &str, timeout_ms: u64) -> Effect {
    Effect::RunCheck { check, argv: vec!["/bin/sh".into(), "-c".into(), script.into()], cwd: cwd.display().to_string(), timeout_ms }
}
...
fn started(dir: &std::path::Path) -> (Group, Pid) {
    ...
    until(|| {
        pids = std::fs::read_to_string(dir.join("pids")).unwrap_or_default()
            .split_whitespace().filter_map(|s| s.parse::<i32>().ok()).collect();
        pids.len() == 2
    });
    let mut pids = pids.into_iter();
    (Group(Pid::from_raw(pids.next().unwrap())), Pid::from_raw(pids.next().unwrap()))
}
...
fn not_running(pid: Pid) -> bool {
    ...
    let output = Command::new("ps").args(["-p", &pid.to_string(), "-o", "stat="]).output().unwrap();
    let status = String::from_utf8_lossy(&output.stdout);
    status.trim().is_empty() || status.trim().starts_with('Z')
}

fn done(adapter: &CheckRunner, rx: &async_channel::Receiver<Inbound>, check: Entity) -> (Option<i32>, String, Option<String>) {
    until(|| !adapter.pending());
    let Inbound::CheckDone { check: owner, code, stdout, problem, .. } = rx.try_recv().unwrap() else { panic!("not CheckDone"); };
...
fn immediate_cancel_and_drop_never_leave_a_queued_spawn_alive() {
    for drop_immediately in [false, true] {
        ...
        runner.apply(spawn(check, dir.path(), HOLD, 60_000)).unwrap();
        runner.apply(Effect::KillCheck { check }).unwrap();
        if drop_immediately {
            drop(runner);
        } else {
...
fn shutdown_and_drop_reap_leader_and_kill_stdout_holding_grandchild() {
    for drop_immediately in [false, true] {
        ...
        if drop_immediately {
            ...
            runner.shutdown();
            assert!(runner.apply(spawn(check, dir.path(), HOLD, 60_000)).is_err());
            let (code, _, problem) = done(&runner, &rx, check);
            assert_eq!(code, None);
            assert!(problem.is_some());
        }
...
fn timeout_kills_inherited_pipe_and_reports_uncertain() {
    ...
    let mut runner = adapter(tx);
    let check = World::new().spawn_empty().id();
    runner.apply(spawn(check, dir.path(), HOLD, 1_000)).unwrap();
    let (leader, descendant) = started(dir.path());
    let (code, _, problem) = done(&runner, &rx, check);
    assert_eq!(code, None);
    assert!(problem.is_some());
...
fn exited_leader_cannot_leave_descendants_holding_stdout() {
    ...
    let mut runner = adapter(tx);
    let check = World::new().spawn_empty().id();
    runner.apply(spawn(check, dir.path(), "sleep 60 & printf '%s %s' \"$PPID\" \"$!\" > pids; printf done; exit 7", 60_000)).unwrap();
    let (leader, descendant) = started(dir.path());
...
fn pending_includes_blocked_completion_delivery() {
    ...
    tx.try_send(Inbound::CheckDone { check: previous, code: Some(0), stdout: String::new(), stderr: String::new(), truncated: false, problem: None }).unwrap();
    let mut runner = adapter(tx);
    runner.apply(spawn(check, dir.path(), "printf '%s %s' \"$PPID\" \"$$\" > pids; printf done", 60_000)).unwrap();
    let (leader, _) = started(dir.path());
    until(|| not_running(leader.0));
    assert!(runner.pending(), "completion has not entered the full inbound queue");
    let Inbound::CheckDone { check: owner, .. } = rx.try_recv().unwrap() else { panic!("not CheckDone"); };
...
fn direct_execute_keeps_normal_output_and_spawn_failure_contract() {
    ...
    let outcome = bevy_tasks::futures_lite::future::block_on(execute(check, &["/bin/sh".into(), "-c".into(), "printf out; printf err >&2; exit 3".into()], "/", 5_000, &registry));
    assert_eq!(outcome.code, Some(3));
    assert_eq!(outcome.stdout, "out");
    assert_eq!(outcome.stderr, "err");
    assert_eq!(outcome.problem, None);
    let outcome = bevy_tasks::futures_lite::future::block_on(execute(check, &["/nonexistent/zor-check".into()], "/", 5_000, &registry));
...
fn host_hard_death_kills_check_descendants() {
    ...
    let mut host = Host(Command::new(std::env::current_exe().unwrap())
        ...
        .env("ZOR_CHECK_HOST_FIXTURE", dir.path()).spawn().unwrap());
    let (leader, descendant) = started(dir.path());
    host.0.kill().unwrap();
    host.0.wait().unwrap();
...
fn check_host_fixture() {
    let Some(dir) = std::env::var_os("ZOR_CHECK_HOST_FIXTURE") else { return; };
    let (tx, _rx) = async_channel::bounded(1);
    let mut runner = adapter(tx);
    let check = World::new().spawn_empty().id();
    runner.apply(spawn(check, std::path::Path::new(&dir), HOLD, 60_000)).unwrap();
