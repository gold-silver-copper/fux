...
use zor::machines::{
    ...
    supervision::{ActionKind, ActionPhase, ActionRecord},
};
use zor::{config::Config, model::Ids};

fn entry(id: &str, name: &str) -> MachineEntry {
    MachineEntry {
        id: id.into(),
        name: name.into(),
        control: None,
...
fn transport(instance: &str, token: &str) -> machines::transport::Transport {
    ...
    machines::transport::Transport::Direct {
        ...
        brp: Descriptor {
            ...
            attach: Some(AttachDescriptor {
                host: "127.0.0.1".into(),
                port: 10,
                token: format!("{token}-attach"),
            }),
            token: token.into(),
        },
    }
...
fn endpoint_credentials_require_admin_and_follow_only_active_bindings() {
    ...
    fn call(
        ...
        let spec = machine_methods::METHODS
            .iter()
            .find(|spec| spec.name == method)
            .unwrap();
        let zor::remote::methods::Handler::Instant(handler) = spec.handler else {
    ...
    configured
        .attachments
        .insert("workspace".into(), attachment.clone());
    ...
    for token in ["read", "plugin"] {
        ...
        for value in [
            ...
            call(
                &mut app,
                "zor/machine.inspect",
                json!({"token":token, "machine":"first"}),
            )
            .unwrap(),
        ] {
    ...
    let resolve = |app: &mut bevy_app::App, machine: &str, workspace: Option<&str>| {
        ...
        .unwrap();
        serde_json::from_value::<machine_methods::MachineEndpoint>(value)
            .unwrap()
            .descriptor
    };
    assert_eq!(
        resolve(&mut app, "first", None),
        control.resolve().unwrap().descriptor
    );
    assert_eq!(
        resolve(&mut app, "m-one", Some("workspace")),
        attachment.resolve().unwrap().descriptor
    );
    assert!(
        call(
            &mut app,
...
fn invalid_reload_retains_entities_and_rename_preserves_stable_identity() {
    ...
    app.update();
    assert_eq!(
        machines::snapshot(app.world(), "first").unwrap().id,
        "m-one"
    );
    catalog.find_mut("m-one").unwrap().name = "renamed".into();
    catalog.save(&path).unwrap();
    ...
    app.update();
    assert_eq!(
        app.world().resource::<Ids>().machine("m-one"),
        Some(original)
    );
    assert!(machines::snapshot(app.world(), "first").is_err());
    assert_eq!(
        machines::snapshot(app.world(), "renamed").unwrap().id,
        "m-one"
    );
...
fn durable_uncertain_actions_survive_restart_and_cannot_be_retargeted() {
    ...
    let uncertain = ActionRecord {
        phase: ActionPhase::Uncertain,
        problem: Some("lost reply".into()),
        ..record
    };
