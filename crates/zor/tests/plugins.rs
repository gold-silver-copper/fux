#![allow(clippy::unwrap_used, clippy::expect_used, clippy::panic, reason = "behavioral regression assertions")]
mod common;
use std::path::Path;
...
fn link(server: &common::Server, path: &Path) -> Value {
    let reply = server.call("zor/plugin.link", json!({"path": path, "enabled": true})).unwrap();
    let until = Instant::now() + Duration::from_secs(10);
    loop {
        let op = server.call("zor/plugin.operation", json!({"operation": reply["operation"]})).unwrap();
        if op["state"] == "complete" { break; }
    ...
    loop {
        let record = server.call("zor/plugin.inspect", json!({"name": manifest.name})).unwrap();
        if record["plugin"]["active"] == true { return record; }
...
fn disable_revokes_authority_and_cancels_all_live_runs() {
    ...
    let descriptor = server.with_world(|world| {
        ...
        fux::remote::client::read_descriptor(&world.get::<plugins::Active>(plugin).unwrap().descriptor).unwrap()
    });
    assert!(fux::remote::client::call_with(&descriptor, "zor/plugin.disable", json!({"name":"owned"})).is_err());
    let first = server.call("zor/plugin.run", json!({"name":"owned","action":"run"})).unwrap();
    let second = server.call("zor/plugin.run", json!({"name":"owned","action":"run"})).unwrap();
    assert_ne!(first["run"], second["run"]);
    let disabled = server.call("zor/plugin.disable", json!({"name":"owned"})).unwrap();
    assert_eq!(disabled["plugin"]["active"], false);
    assert!(fux::remote::client::call_with(&descriptor, "zor/server.info", json!({})).is_err());
    assert!(server.call("zor/plugin.run", json!({"name":"owned","action":"run"})).is_err());
    server.with_world(|world| {
        let plugin = plugins::find(world, "owned").unwrap();
        assert!(plugins::record(world, plugin).runs.iter().all(|r| matches!(r.state, RunState::Exited { .. } | RunState::Stopping)));
    });
}
...
fn cursor_grants_cannot_claim_another_plugin_and_failed_persistence_keeps_memory() {
    ...
    fixture(&a, "alpha"); fixture(&b, "beta");
    link(&server, &a); link(&server, &b);
    let descriptor = server.with_world(|world| {
        let plugin = plugins::find(world, "alpha").unwrap();
        fux::remote::client::read_descriptor(&world.get::<plugins::Active>(plugin).unwrap().descriptor).unwrap()
    });
    assert!(fux::remote::client::call_with(&descriptor, "zor/plugin.cursor", json!({"name":"beta","zor":99})).is_err());
    server.with_world(|world| {
        let plugin = plugins::find(world, "alpha").unwrap();
...
fn imports_report_invalid_manifest_without_replacing_last_valid_plugin() {
    ...
    let submitted = server.call("zor/plugin.link", json!({"path":dir.path(),"enabled":true})).unwrap();
    let until = Instant::now() + Duration::from_secs(10);
    loop {
        let op = server.call("zor/plugin.operation", json!({"operation":submitted["operation"]})).unwrap();
        if op["state"] == "failed" { break; }
        assert!(Instant::now() < until);
        std::thread::sleep(Duration::from_millis(10));
    }
    let inspected = server.call("zor/plugin.inspect", json!({"name":"valid"})).unwrap();
    assert_eq!(inspected["plugin"]["version"], "1");
    assert!(server.call("zor/plugin.run", json!({"name":"valid","action":"run"})).is_ok());
}
...
fn killing_before_adapter_spawn_never_executes_command() {
    ...
    let mut adapter = plugins::host::PluginAdapter::with_executable(send, env!("CARGO_BIN_EXE_zor").into());
    let plugin = bevy_ecs::world::World::new().spawn_empty().id();
    let marker = dir.path().join("executed");
    adapter.apply(Effect::RunPlugin { plugin, run:1, argv:vec!["/bin/sh".into(), "-c".into(), format!("sleep 2; touch '{}'", marker.display())], cwd:None, env:vec![], log:dir.path().join("log") }).unwrap();
    adapter.apply(Effect::KillPlugin { plugin, run:1 }).unwrap();
    let until = Instant::now() + Duration::from_secs(8);
    loop {
        if let Ok(Inbound::PluginExited { run:1, .. }) = recv.try_recv() { break; }
...
fn repeated_stars_match_without_exponential_backtracking() {
    ...
    assert!(!plugins::manifest::pattern_matches(&pattern, &format!("zor/{}y", "a".repeat(100))));
    assert!(plugins::manifest::pattern_matches("zor/*Closed", "zor/TaskClosed"));
    assert!(!plugins::manifest::pattern_matches("zor/*Closed", "fux/PaneClosed"));
}
...
fn binary_hot_output_keeps_recent_bounded_logs() {
    ...
    for _ in 0..80 {
        plugins::host::append_line(&log, "[run] ", &vec![b'x'; plugins::host::MAX_LINE_BYTES * 2]).unwrap();
    }
    ...
    assert_eq!(tail, vec!["[run] binary \u{fffd} output", "[run] final marker"]);
    let bound = plugins::host::MAX_LOG_BYTES + plugins::host::MAX_LINE_BYTES as u64 + 1024;
    assert!(std::fs::metadata(&log).unwrap().len() <= bound);
    assert!(std::fs::metadata(log.with_extension("log.1")).unwrap().len() <= bound);
}
...
impl PaneHost {
    fn new() -> Self {
        ...
        app.world_mut().insert_resource(plugins::FuxDescriptor(fux.brp.clone()));
        app.finish();
        app.cleanup();
        app.update();
        let source = dir.path().join("plugin");
        std::fs::create_dir(&source).unwrap();
        std::fs::write(source.join("zor-plugin.toml"), r#"
name = "panes"
version = "1"
[[actions]]
id = "busy"
...
"#).unwrap();
        ...
        let plugin = loop {
            ...
            if let Ok(plugin) = plugins::find(app.world(), "panes")
                && app.world().get::<plugins::Active>(plugin).is_some() { break plugin; }
        ...
        Self { app, fux, effects: Default::default(), adapter, replies, plugin, _dir: dir }
    }
    ...
    fn step(&mut self) {
        ...
        use zor::runner::Adapter;
        self.app.update();
        let effects: Vec<_> = self.effects.read(self.app.world().resource::<bevy_ecs::message::Messages<Effect>>())
            .filter_map(|effect| match effect {
                Effect::FuxCall { call, method, params } => Some((*call, method.clone(), params.clone())),
                _ => None,
            }).collect();
        for (call, method, params) in effects {
            ...
            self.adapter.apply(Effect::FuxCall { call, method, params }).unwrap();
            self.app.world_mut().write_message(self.replies.recv_blocking().unwrap());
        }
    }
    ...
    fn until(&mut self, condition: impl Fn(&Self) -> bool) {
        ...
        while !condition(self) {
            assert!(Instant::now() < deadline, "pane transition timed out: {:?}", plugins::record(self.app.world(), self.plugin));
            self.step();
    ...
    fn open(&mut self, kind: &str) -> u64 {
        plugins::open_pane(self.app.world_mut(), self.plugin, kind, "default", None, None).unwrap()
    }

    fn pane(&self, id: u64) -> plugins::OpenPane {
        self.app.world().get::<plugins::Panes>(self.plugin).unwrap().0.iter().find(|p| p.id == id).unwrap().clone()
    }
}
...
fn closed_owned_surface_panes_reuse_capacity_and_revoke_grants() {
    ...
    let initial = host.fux.call("fux/server.info", json!({})).unwrap()["tokens_minted"].as_u64().unwrap();
    for _ in 0..(plugins::MAX_RUNS_RETAINED + 2) {
        let opening = host.open("surface");
        host.until(|h| h.app.world().get::<plugins::Runs>(h.plugin).unwrap().0.iter()
            .any(|r| r.surface.is_some() && r.state == RunState::Running));
        let run = host.app.world().get::<plugins::Runs>(host.plugin).unwrap().0.iter()
            .find(|r| r.surface.is_some() && r.state == RunState::Running).unwrap().id;
        host.app.world_mut().write_message(zor::model::Inbound::PluginExited { plugin: host.plugin, run, code: Some(0) });
        host.until(|h| matches!(h.pane(opening).state, plugins::PaneState::Closed));
        host.step();
        assert_eq!(host.fux.call("fux/server.info", json!({})).unwrap()["tokens_minted"], initial);
    }
}
...
fn stale_surface_driver_exit_preserves_replacement_provider() {
    ...
    host.until(|h| h.app.world().get::<plugins::Runs>(h.plugin).unwrap().0.iter()
        .any(|r| r.surface.is_some() && r.state == RunState::Running));
    let run = host.app.world().get::<plugins::Runs>(host.plugin).unwrap().0.iter()
        .find(|r| r.surface.is_some()).unwrap().id;
    let plugins::PaneState::Open { surface: Some(surface), .. } = host.pane(opening).state else { unreachable!() };
    let closed = host.fux.call("fux/surface.close", json!({"surface":surface})).unwrap();
    host.fux.call("fux/surface.open", json!({"workspace":"default","node":surface,
        "provider":"replacement","generation":closed["generation"]})).unwrap();
    host.app.world_mut().write_message(zor::model::Inbound::PluginExited { plugin:host.plugin, run, code:Some(0) });
    for _ in 0..4 { host.step(); }
    assert!(matches!(host.pane(opening).state, plugins::PaneState::Open { .. }));
    let provider = host.fux.with_world(move |world| {
        let node = world.resource::<fux::model::Ids>().node(fux::model::NodeId(surface)).unwrap();
        world.get::<fux::surface::SurfaceState>(node).unwrap().provider.clone()
    });
...
fn surface_driver_refusal_removes_owned_surface_and_container() {
    ...
    for _ in 0..plugins::MAX_RUNS_RETAINED {
        plugins::run_action(host.app.world_mut(), host.plugin, "busy", plugins::RunContext::default()).unwrap();
    }
    host.until(|h| h.app.world().get::<plugins::Runs>(h.plugin).unwrap().0.iter().all(|r| r.state == RunState::Running));
    let initial = host.fux.call("fux/server.info", json!({})).unwrap()["tokens_minted"].clone();
    let nodes = host.fux.with_world(|world| world.query::<&fux::model::NodeId>().iter(world).count());
    let opening = host.open("surface");
    host.until(|h| matches!(h.pane(opening).state, plugins::PaneState::Failed { .. }));
    let failed = host.pane(opening);
    assert!(matches!(failed.state, plugins::PaneState::Failed { ref problem } if problem.contains("live run limit")));
    ...
    assert_eq!(host.fux.with_world(|world| world.query::<&fux::model::NodeId>().iter(world).count()), nodes);
}
...
fn terminal_closure_requires_same_incarnation_evidence() {
    ...
    host.until(|h| matches!(h.pane(opening).state, plugins::PaneState::Open { pane: Some(_), .. }));
    let plugins::PaneState::Open { pane: Some(pane), .. } = host.pane(opening).state else { unreachable!() };
    host.app.world_mut().write_message(zor::model::Inbound::FuxLink { instance: Some("replacement".into()) });
    host.app.world_mut().write_message(zor::model::Inbound::FuxEvent { cursor: 1, name: "PaneClosed".into(), body: json!({"pane":pane}) });
    host.step();
    assert!(matches!(host.pane(opening).state, plugins::PaneState::Open { .. }));
    ...
    host.fux.call("fux/pane.close", json!({"pane":pane})).unwrap();
    // This HTTP fixture has no PTY adapter: closing requests termination but is not exit evidence.
    host.fux.with_world(move |world| {
        world.write_message(fux::model::Inbound::PaneExited { pane: entity, code: 0 });
    });
    host.app.world_mut().write_message(zor::model::Inbound::FuxLink { instance: Some(host.fux.descriptor.instance.clone()) });
    host.until(|h| matches!(h.pane(opening).state, plugins::PaneState::Closed));
}
...
fn activation_cursor_survives_disconnect_before_first_event_without_replay() {
    ...
    let (viewer, viewer_id) = fux.with_world(|world| {
        let workspace = world.resource::<fux::model::Ids>().workspace("default").unwrap();
        let viewer = fux::layout::ops::attach_viewer(
            world, workspace, fux::model::Viewport { rows: 24, cols: 80 }, None,
        ).unwrap();
    ...
    let (zor_brp, cursors) = server.with_world(|world| {
        ...
        (world.get::<plugins::Active>(plugin).unwrap().descriptor.clone(),
    ...
    let _child = Child(std::process::Command::new(env!("CARGO_BIN_EXE_zor"))
        .args(["plugin", "hook", "hook"])
        .env("ZOR_PLUGIN_NAME", "hook").env("ZOR_BRP", zor_brp).env("FUX_BRP", &fux.brp)
        .env("ZOR_PLUGIN_CURSORS", serde_json::to_string(&cursors).unwrap())
        .env("ZOR_PLUGIN_HOOKS", json!([{"pattern":"fux/*","command":["/bin/sh","-c",
            "printf '%s\\n' \"$ZOR_PLUGIN_EVENT_CURSOR\" >> \"$1\"", "hook", marker]}]).to_string())
        .stdin(std::process::Stdio::null()).stdout(std::process::Stdio::null()).spawn().unwrap());
    let wait_for = |condition: &dyn Fn() -> bool| {
        let deadline = Instant::now() + Duration::from_secs(10);
        while !condition() {
            assert!(Instant::now() < deadline, "hook did not reach expected observation");
            std::thread::sleep(Duration::from_millis(10));
        }
    };
    wait_for(&|| fux.call("fux/server.info", json!({})).unwrap()["watches"].as_u64().unwrap() > 0);
    ...
    let latest = fux.call("fux/events.poll", json!({"cursor":null})).unwrap()["cursor"].as_u64().unwrap();
    wait_for(&|| std::fs::read_to_string(&marker).unwrap_or_default().lines().any(|line| line.parse::<u64>().ok() == Some(latest)));
    let events = std::fs::read_to_string(&marker).unwrap();
    let dispatched: Vec<u64> = events.lines().map(|line| line.parse().unwrap()).collect();
    assert!(dispatched.iter().all(|cursor| *cursor > baseline), "preactivation history replayed: {events}");
    assert!(dispatched.windows(2).all(|pair| pair[0] < pair[1]), "duplicate dispatch: {events}");
}
