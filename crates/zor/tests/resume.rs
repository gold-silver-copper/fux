...
fn live_with_spec(world: &mut World, spec: LaunchSpec) -> (Entity, Entity, Entity) {
    ...
    let prompt = lifecycle::prepare_prompt(
        ...
        attempt,
        PromptSpec {
            id: "original-prompt".into(),
            text: "never replay this prompt".into(),
            timeout_ms: 30_000,
        },
    )
    .unwrap();
    world.entity_mut(prompt).insert((
        ...
        Binding {
            producer: "opencode:observer".into(),
            sequence: 2,
            input_operation: 42,
            session: SESSION.into(),
            message: "msg_original".into(),
            bound_ms: 200,
        },
        ResponseEvent {
            producer: "opencode:observer".into(),
    ...
    effects
        ...
        .filter_map(|effect| match effect {
            Effect::FuxCall {
                ...
                method,
                params,
            } if method == "fux/root.new" || method == "fux/workspace.new" => {
...
fn stable_resume_retry_is_retained_and_conflicting_identity_is_refused() {
    ...
    assert!(matches!(
        resume(world, task, changed),
        Err(LifecycleError::Conflict(_))
    ));
    assert!(matches!(
        resume(world, task, request("original")),
        Err(LifecycleError::Conflict(_))
    ));
    assert!(matches!(
        resume(world, task, request("original-prompt")),
        Err(LifecycleError::Conflict(_))
    ));
...
fn live_lost_and_unproven_sidecar_exit_never_authorize_a_new_attempt() {
    ...
    lifecycle::observe(
        world,
        old,
        Observed::Replaced("another fux incarnation".into()),
    );
    assert!(resume(world, task, request("resume-1")).is_err());
    lifecycle::observe(
        world,
...
fn unsupported_provider_and_arbitrary_launch_commands_are_not_resume_policy() {
    ...
    let original = world
        .get::<lifecycle::LaunchTemplate>(operation)
        .unwrap()
        .clone();
    let mut prompt_command = original.clone();
    prompt_command
        .argv
        .extend(["run".into(), "replay an old prompt".into()]);
...
fn lost_reply_and_journal_restart_preserve_resume_uncertainty_without_recreation() {
    ...
    let new_id;
    {
        ...
        world.write_message(Inbound::FuxReply {
            call,
            result: Err("connection lost after dispatch".into()),
        });
        app.update();
        let world = app.world_mut();
...
fn guarded_endpoint_distinguishes_zor_envelope_from_fux_target_and_allows_stable_retry() {
    ...
    let status = server
        .call("zor/task.resume-status", json!({ "task": "task" }))
        .unwrap();
...
fn late_ephemeral_cleanup_cannot_target_the_resumed_workspace() {
    ...
    let pending_cleanup = effects(world)
        .into_iter()
        .find_map(|effect| match effect {
            Effect::FuxCall { method, params, .. } if method == "fux/workspace.kill" => {
                Some(params)
            }
            _ => None,
        })
    ...
    assert_eq!(
        new_workspace,
        &world.get::<PaneHandle>(attempt).unwrap().workspace
    );
    assert_eq!(resume(world, task, request("resume-1")).unwrap(), attempt);
    assert!(effects(world).is_empty());
}
...
fn sidecar_exit_after_finished_commit_preserves_native_authority_across_restart() {
    let dir = tempfile::tempdir().unwrap();
    {
        ...
        app.update();
        assert_eq!(
            eligibility(app.world(), task).unwrap().native_session,
            SESSION
        );
    }
    let mut app = build(dir.path());
