...
use zor::model::{Effect, Location};
use zor::runner::{Adapter, ZorHost};

struct Recording(Arc<AtomicUsize>);
impl Adapter for Recording {
    fn handles(&self, effect: &Effect) -> bool { matches!(effect, Effect::FuxCall { .. }) }
    fn apply(&mut self, effect: Effect) -> Result<(), bevy_ecs::error::BevyError> {
        if matches!(effect, Effect::FuxCall { .. }) { self.0.fetch_add(1, Ordering::SeqCst); }
        Ok(())
    }
}
...
fn launch_and_run(fail_commit: bool) -> (AppExit, usize, tempfile::TempDir) {
    ...
    let task = lifecycle::create_task(app.world_mut(), TaskSpec {
        id:"durability".into(), title:"durable launch".into(), location: Location::Cwd(dir.path().display().to_string()),
    }).unwrap();
    lifecycle::launch(app.world_mut(), task, LaunchSpec {
        operation:"launch-once".into(), argv:vec!["/bin/true".into()], env:vec![],
        workspace:"default".into(), ephemeral:false, integration:None,
    }).unwrap();
    if fail_commit {
    ...
    let host = ZorHost::new(app.world_mut(), vec![Box::new(Recording(Arc::clone(&calls)))]);
