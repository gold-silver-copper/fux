...
fn revision(app: &App, viewer: Entity) -> u64 {
    app.world().get::<ProjectionBaseline>(viewer).unwrap().scene_revision
}
...
fn delayed_click_and_key_cannot_name_the_row_now_occupying_the_old_cell() {
    ...
    let viewer = ops::attach_viewer(app.world_mut(), ws, Viewport { rows: 24, cols: 80 }, None).unwrap();
    ...
    provider.world.entity_mut(column).replace_children(&[rows[1], rows[0]]);
    let delta = provider.export_all();
    ...
    request(&mut app, viewer, click_at(painted, 0));
    request(&mut app, viewer, ViewerRequest::SurfaceKey {
        revision: painted, node: surface_id, bytes: b"s".to_vec(),
    });
    assert!(surface_inputs(app.world()).is_empty());
    ...
    let row_b = app.world().get::<SurfaceState>(leaf).unwrap().entity(rows[1]).unwrap();
...
fn queued_surface_key_cannot_cross_provider_replacement() {
    ...
    request(&mut app, viewer, ViewerRequest::SurfaceKey {
        revision: old, node, bytes: b"s".to_vec(),
    });
    assert!(surface_inputs(app.world()).is_empty());
    let current = revision(&app, viewer);
    request(&mut app, viewer, ViewerRequest::SurfaceKey {
        revision: current, node, bytes: b"j".to_vec(),
    });
    let inputs = surface_inputs(app.world());
    assert_eq!(inputs.len(), 1);
...
fn scroll_and_resize_refuse_old_clicks_without_invalidating_another_viewer() {
    ...
    let template_column = app.world().get::<SurfaceState>(leaf).unwrap().entity(column).unwrap();
    ops::scroll(app.world_mut(), a, template_column, 1).unwrap();
    request(&mut app, a, click_at(old_a, 0));
    assert!(surface_inputs(app.world()).is_empty());
    let node = NodeId(node_id(app.world(), leaf));
    request(&mut app, b, ViewerRequest::SurfaceKey {
        revision: old_b, node, bytes: b"j".to_vec(),
    });
    let inputs = surface_inputs(app.world());
    assert_eq!(inputs.len(), 1);
    assert_eq!(inputs[0]["viewer"], json!(app.world().get::<ViewerId>(b).unwrap().0));
...
fn terminal_output_frames_preserve_surface_admission_and_pty_typing() {
    ...
    let painted = revision(&app, viewer);
    for _ in 0..3 {
        app.world_mut().write_message(Inbound::PaneOutput { pane, bytes: b"busy output\r\n".to_vec() });
        app.update();
    }
    let node = NodeId(node_id(app.world(), leaf));
    request(&mut app, viewer, ViewerRequest::SurfaceKey {
        revision: painted, node, bytes: b"j".to_vec(),
    });
    let inputs = surface_inputs(app.world());
    assert_eq!(inputs.len(), 1);
    assert_eq!(inputs[0]["bytes"], json!([106]));
    app.world_mut().resource_mut::<Messages<Effect>>().drain().for_each(drop);
