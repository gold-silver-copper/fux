...
fn mixed_terminal_and_replacement_frames_keep_old_surface_key_provenance() {
    ...
    viewer::update(&mut app);
    assert_eq!(take_requests(&mut app), vec![ViewerRequest::SurfaceKey {
        revision: 1, node: NodeId(3), bytes: b"s".to_vec(),
    }]);
    ...
    viewer::update(&mut app);
    assert_eq!(take_requests(&mut app), vec![ViewerRequest::Input(b"y".to_vec())]);
}
...
fn continuous_surface_input_does_not_defer_new_paints() {
    ...
    take_requests(&mut app);
    for revision in 1..=8 {
        viewer::queue_terminal(app.world_mut(), key(TKey::Char('j'), Modifiers::NONE), revision);
        push_frame(&mut app, scene.frame(revision + 1));
        viewer::update(&mut app);
        assert_eq!(take_requests(&mut app), vec![ViewerRequest::SurfaceKey {
            revision, node: NodeId(3), bytes: b"j".to_vec(),
        }]);
        assert_eq!(app.world().resource::<PaintedSceneRevision>().0, revision + 1);
...
fn output_only_frames_do_not_discard_queued_terminal_or_surface_typing() {
    ...
    viewer::queue_terminal(app.world_mut(), key(TKey::Char('x'), Modifiers::NONE), 1);
    viewer::update(&mut app);
    assert_eq!(take_requests(&mut app), vec![ViewerRequest::Input(b"x".to_vec())]);
    chord(&mut app, 'l');
    take_requests(&mut app);
    viewer::queue_terminal(app.world_mut(), key(TKey::Char('j'), Modifiers::NONE), 1);
    viewer::update(&mut app);
    assert_eq!(take_requests(&mut app), vec![ViewerRequest::SurfaceKey {
        revision: 1, node: NodeId(3), bytes: b"j".to_vec(),
    }]);
}
