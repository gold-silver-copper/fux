#!/bin/sh
# BRP policy work, finding 025 (class 6).
#
# Every workspace carries a WorkspaceOrder, all distinct: it is what orders
# the workspace list and what reordering swaps. BRP could spawn a Workspace
# without one, or give two workspaces the same one, and fux kept both.
#
# Exit 0: reproduced (a workspace without an order, or two sharing one).
# Exit 1: verified not reproduced.
# Exit 2: setup failure.
# NEGATIVE_CONTROL=1 creates the workspace through the workspace_new command.
#
# Usage: 025-workspace-order-can-be-missing-or-shared.sh /path/to/fux
set -u
...
DIR="$(mktemp -d /tmp/fux-repro-025.XXXXXX)" || exit 2
cleanup() {
...
if CONTROL:
    control({"kind": "workspace_new", "name": "second"})
else:
    rpc("world.spawn_entity", {"components": {"fux::model::Workspace": {}}})
settle()
orders = []
missing = 0
for row in query("fux::model::Workspace"):
    order = component(row["entity"], "fux::model::WorkspaceOrder")
    if order is None:
        missing += 1
    else:
        orders.append(order)
print("workspaces without an order:", missing, "| orders:", orders)
if missing or len(set(orders)) != len(orders):
    print("REPRODUCED: workspace order is missing or shared")
    sys.exit(10)
sys.exit(20)
PY
RESULT=$?
