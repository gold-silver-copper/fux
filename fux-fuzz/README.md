...
# Smoke: all forty-three scenarios (sixty-five isolated cases, including one generated walk).
...
fux-fuzz/target/debug/fux-fuzz --fux target/debug/fux --scenario transport
...
- **Raw:** ordinary steps interleaved with the raw API mutations the README permits but does not own: despawning an API viewer, a tab, a split container, a pane view or a process entity; removing `Viewing`, `OnTab`, `Focused`, `WorkspaceOrder`, `Children`, a tab's `ChildOf`, `Viewer`, `Launch` or `PaneView`; inserting a `Viewing` at a despawned entity or at a pane view, a `Focused` at a tab, an `OnTab` at any tab, a `Viewer` of 0x0 or 4096x4096, a tab's `ChildOf` at one of its own panes and at itself; and partial payloads for `Viewer`, `Launch`, `PaneView` and `Prefix` that omit a required field, which must be rejected naming that field and change nothing (the agent exercises' F1, once a server-ending panic). The oracle is narrower and stated per mutation: the server never panics or hangs, every remaining viewer keeps painting within its viewport, the driver's relationships are repaired as the README promises after despawns and dangling or mistyped targets, and the next ordinary step succeeds or reports a documented notice, including after a tab is unlinked from its workspace.
...
- **Transport:** the HTTP transport fux owns since PR #45, spoken to with raw Unix sockets instead of a cooperative client: a GET and an OPTIONS with no body, HTTP/1.0, a path that is not `/`, `Expect: 100-continue`, `Content-Length` with `Transfer-Encoding`, a well formed and a malformed chunked body, a short `Content-Length`, a negative one, invalid UTF-8 in the body, the target and a header, a NUL in a header, bare LF line endings, bytes that are not HTTP at all, an empty body, `null`, a number, `[]`, unparsable JSON, a request with no method, an object id, an unknown method, a 1000-request batch, 10 000 headers, a 1 MiB header, params nested 10 000 deep, a 1 MiB id, ten pipelined requests, an idle connection, and a watch inside a batch. It also drives the transport's limits: a request body and a batch past their caps, each refused with a typed error naming the limit and the refusal itself small. Each case opens its own connection and closes it; after every one the next ordinary request on a fresh connection must succeed and the driver must still paint. All of these are correctly refused or absorbed, so this scenario is coverage.
- **Identity:** every surface that lets a request name an entity, against every value class: bits whose low 32 bits are zero, the opaque bits of entity indices 0, 1 and 2, `u64::MAX`, a never-allocated id, a despawned id, an id of the wrong kind, the viewer itself, a live pane view, and a negative number, a float, a string, null, a bool, an array and an object. The surfaces are `fux.frame`, `fux.frame+watch`, the `Control` and `UserInput` event targets, ten command subjects, `load_layout` mappings, five entity-typed component fields, `Children`, `Overlay.target`, and six stock methods. After each value the server must still answer and the driver must still paint. A viewer that a layout component legitimately removes is replaced and the sweep carries on; the driver is an API viewer rather than a frontend for that reason. It closes by watching a freshly made tab, workspace and pane view and requiring each to survive, which is finding 003's ground. It fails against a binary carrying finding 003, 004 or 005; see [BREAKS.md](BREAKS.md).
- **Hostile:** the hunt 5 inputs that once aborted the server (see [BREAKS.md](BREAKS.md)): a `Viewer` inserted onto a tab, then a viewer relationship inserted onto a real viewer. The server must keep answering, the tab must no longer carry `Viewer`, and the real viewer's workspace, tab and focused pane must still belong together.
...
Each server listens on its own Unix socket (`--socket`) in a fresh mode-0700 directory under the system temporary directory, kept short because a socket path is limited to about 100 bytes; frontends receive it as `FUX_SOCKET`. No other process can own that path, so there is no collision to classify and no retry loop. Readiness waits for the initial pane before any mutation. All BRP requests of one server share one pooled connection agent.
