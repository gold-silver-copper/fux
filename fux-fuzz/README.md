...
# Smoke: all forty-three scenarios (sixty-four isolated cases, including one generated walk).
...
# Registered but excluded from the smoke: reproduces an unfixed break (hunt 6).
...
- **Identity** (excluded from `all`): every surface that lets a request name an entity, against every value class: bits whose low 32 bits are zero, the opaque bits of entity indices 0, 1 and 2, `u64::MAX`, a never-allocated id, a despawned id, an id of the wrong kind, the viewer itself, a live pane view, and a negative number, a float, a string, null, a bool, an array and an object. The surfaces are `fux.frame`, `fux.frame+watch`, the `Control` and `UserInput` event targets, ten command subjects, `load_layout` mappings, five entity-typed component fields, `Children`, `Overlay.target`, and six stock methods. After each value the server must still answer and the driver must still paint. This reproduces finding 003 and fails on purpose; see [BREAKS.md](BREAKS.md).
