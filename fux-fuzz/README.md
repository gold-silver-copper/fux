...
**The startup smoke exposed a first-pane configuration bug in base `cac0def02caedad8c60b6bd4357e1be013999da1`, now fixed in this PR.** The original saved failure trace passes against the fixed binary without changing the harness assertion. Exit 1 means at least one scenario, setup, diagnostic, cleanup, interruption, or deadline failed. Other cases still execute within the overall budget; no failed case is retried. A dependency panic during scenario execution is reported as a harness/dependency failure, not an application defect.
