...
| `walk-shrunk-pane-loses-its-cells` | a 10x40 viewer, one stacked split, four shrinks | The flex weight floor of 0.1 let a shrunk pane round to no cell beside its sibling, so it painted nothing yet kept focus and input; the two-cell rule held only at split time. | Pane nodes carry a 2x2 minimum and containers an automatic one, so a shrunk pane keeps two cells and a viewer too small for its panes overflows at the end of an axis, never over a neighbour. |
