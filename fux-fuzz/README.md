...
Observed and not counted: a pane whose flex weight was shrunk to 0.1 by resize commands can round to no cell at all beside larger siblings in a small viewer; the split rule promises two cells at split time in the requesting viewer, not afterwards. A tab unlinked from its workspace by a raw hierarchy edit stays orphaned and a viewer on it stays there; the README promises repair only for despawned targets.
