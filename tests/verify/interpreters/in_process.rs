...
impl Interpreter for InProcessInterpreter {
    fn run(&self, scenario: &Scenario) -> Result<Vec<Entry>, String> {
        ...
        let mut workspace_factory = WorkspaceFactory;
        ...
        for step in &scenario.steps {
            match step {
                Step::StartDaemon => {
                    ...
                    daemon = Some(
                        fux::daemon::Daemon::new(paths, std::process::id(), 0)
                            .map_err(|error| error.to_string())?,
                    );
                    daemon
                        .as_mut()
                ...
                Step::ChildOutput { pane, bytes } => {
                    ...
                    let frame = ExpectedTerminalFrame {
                        ...
                        // Copy belongs to the viewer and does not change shared pane semantics.
                        selection: None,
                ...
                Step::TerminalReply { pane, query, bytes } => {
                    ...
                    let mut terminal =
                        fux::terminal::ServerTerminal::new(terminal_size.0, terminal_size.1, 0);
