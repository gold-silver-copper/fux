...
- `navigation.rs`: hierarchy normalization and viewer memory. Component hooks record the tab per workspace and the focused and previously focused pane per tab as relationships change, and queue repair on insertion/removal. Server-scoped observers still prune dead memory entries and normalize hierarchy changes: a workspace that loses its last tab gets one back, a child placed directly under a workspace is wrapped into a tab, a `ChildOf` that points into the entity's own subtree is rejected like self-parenting rather than left as a cycle, and a tab unlinked from its workspace or parented elsewhere has its viewers repaired onto a listed tab. Normalization/pruning observers are not installed in inert presentation worlds. Nothing scans for dangling IDs on a schedule.
