...
The uppercase IDs above are placeholders to substitute, not literal JSON values. `UserInput` also accepts `paste_begin` followed by `paste {text}` (ownership-preserving fragmented paste), atomic `paste {text}`, `resize {rows,cols}` and `mouse {action,button,x,y,ctrl,alt,shift}` with a `kind` discriminator; mouse coordinates are zero-based viewer cells. Control/file-operation errors appear in the actual reflected `Viewer.notice`; asynchronous scene completion changes that notice. `Viewer` has no `prompt` or `buffer` fields: text prompts are overlay state, and `help_scroll` is the prefix column selection. A `Control` whose `action` is not a known action name is rejected when the request deserializes, with a JSON-RPC error instead of a notice. `Viewer` also no longer serializes `prefix` or `help_scroll`: the open command column is unreflected viewer state, observable only in painted frames.
