...
Click to focus. Application mouse protocols are forwarded in pane-relative coordinates, including the first and last content cells; Shift-wheel scrolls local history instead. Bar/separator clicks are not pane input. Copy requires an outer terminal that accepts OSC 52. Prefix, prompts and help consume keys, paste and pointer events without click-through; list entries are not clickable. Ordinary keys and bracketed paste go to the focused PTY.

Panes are borderless: content starts at the top-left cell, with one shared thin separator between default split siblings. Separators next to focus are bold; others are muted. The last row is always a full-width, gray-background bar: workspace on the left, focused process `id: name`/exit status on the right. Notices replace the right zone (yellow; errors red) until subsequent input clears them. Zoom/history indicators stay compact. Unfocused exited panes retain a small dim marker, not a title strip.

The command/help list grows upward from the bottom-right, directly above the bar, one configured binding per row in configuration order. A bold heading, contrasting background and minimal padding distinguish it without a border. Hidden rows are marked `▲ n more` / `▼ n more` when space permits; narrow labels use cell-aware ellipsis. Rename, workspace-move and scene-path prompts use the same corner surface, with the editable text reversed. Closing an overlay restores the pane and cursor. A one-row viewer shows only the bar; zero-sized views paint no content.
