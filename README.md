...
- `server.rs`: direct typed Bevy observers, queries/resources and scheduled systems implement operations. Exclusive scene preparation is kept separate from a cached native sizing/paint system, which borrows Viewer state instead of cloning it; independent non-send presentation Apps retain native focus isolation. A causal settling update lets stock `RemoteLast` mutations reach lifecycle/layout systems even when idle. Workspace-owned layout caches follow arbitrary component change ticks, archetype changes and native hierarchy membership, not a component allowlist. Unrelated workspace projections remain cached; removing a Viewer also removes its presentation context.
...
Tested on macOS arm64; see [REFINEMENT.md](REFINEMENT.md) for current comparable measurements and the complete capability audit, and [VERIFICATION.md](VERIFICATION.md) for the preserved baseline evidence. Linux and other Unix systems are unvalidated; this is not a Windows/mobile implementation.
