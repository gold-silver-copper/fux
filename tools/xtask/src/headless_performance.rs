        ...
        while pending.len() >= 4 {
            ...
            pending.drain(..4 + length);
            {
                ...
                if let Some(frame) = value.get("state") {
                    c.frames += 1;
                    c.latest = frame.get("state").context("frame state")?.clone();
                } else {
                    ensure!(
                        value == json!({"hello":{"version":6}}),
...
impl Viewer {
    ...
    fn visible(&self, marker: &str, count: usize) -> Result<bool> {
        ...
        let Some(panes) = c.latest.get("panes").and_then(Value::as_object) else {
            return Ok(false);
        };
        if panes.len() != count {
            return Ok(false);
        ...
        for p in panes.values() {
            ...
            for cell in p["cells"].as_array().context("cells")? {
                text.push_str(cell["text"].as_str().context("cell text")?);
            }
...
mod tests {
    ...
    fn reader_decodes_fragmented_frames_and_counts_received_bytes() -> Result<()> {
        ...
        for v in [
            json!({"hello":{"version":6}}),
            json!({"state":{"state":{"panes":{"1":{"cells":[{"text":"DONE"}]}}}}}),
        ] {
        ...
        let observed = local::until(Duration::from_secs(2), || {
            ...
            Ok((c.frames == 1 && c.decoded == bytes.len() as u64).then_some(c.value()))
        });
