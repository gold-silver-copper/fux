//! Fixture-only transparent FUXCTL3 traffic accounting, never linked to production.
...
mod tests {
    ...
    fn forwards_capture_and_subscription_bytes_and_stops_idle_handlers() -> Result<()> {
        ...
        let backend = thread::spawn(move || -> Result<()> {
            for command in ["capture", "subscribe"] {
                ...
                let state = State::default();
                    ...
                    line(&mut peer, 8, &state)? == b"FUXCTL3\n",
                    "preface forwarding"
                );
                write(&mut peer, b"FUXCTL3\n", &state)?;
                let request = line(&mut peer, 65536, &state)?;
        ...
        let outcome = (|| -> Result<()> {
            ...
            let state = State::default();
            write(&mut client, b"FUXCTL3\n", &state)?;
            ensure!(
                line(&mut client, 8, &state)? == b"FUXCTL3\n",
                "client preface"
            );
