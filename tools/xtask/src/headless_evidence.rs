...
fn hash(root: &Path, path: &str, expected: &Value) -> Result<()> {
    ...
    if path == "tools/headless_performance.py"
        && wanted == "a528eaa007ad7ccaf61b00a27028c72224399e8327843f3b902fdf9fff2e1e7f"
    {
        let archive = root.join("tools/archive/tools/headless_performance.py.txt");
        ensure!(
            format!("{:x}", Sha256::digest(fs::read(archive)?)) == wanted,
            "historical performance source hash"
        );
        return Ok(());
    }
    if path == "tools/headless_journal.py" {
        let archive = root.join("tools/archive/tools/headless_journal.py.txt");
        if format!("{:x}", Sha256::digest(fs::read(archive)?)) == wanted {
            return Ok(());
        }
    }
    // The retained sharing experiment predates the test-only N3 cost probe.
    // Verify its exact original bytes, never substitute a new current-source hash.
    if path == "src/view.rs"
        && wanted == "e558e0e2d25d67445438b9fe287f65b2952e821d9889eb97e0458b61110a58ee"
    {
        let archive = root.join("tools/archive/native-milestone/view.rs.txt");
        ensure!(
            format!("{:x}", Sha256::digest(fs::read(archive)?)) == wanted,
            "historical pane-view source hash"
        );
        return Ok(());
    }
    ensure!(
...
pub fn regressions() -> Result<()> {
    ...
    let read = |name: &str| -> Result<Value> {
        Ok(serde_json::from_slice(&fs::read(
            root.join("docs/headless-performance").join(name),
        )?)?)
    };
...
mod tests {
    ...
    fn rust_capture_requires_every_source_hash() -> Result<()> {
        ...
        let mut fixture: Value = serde_json::from_slice(&fs::read(
            root.join("docs/headless-performance/fux-before.json"),
        )?)?;
