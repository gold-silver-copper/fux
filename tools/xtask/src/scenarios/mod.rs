...
fn empty_arguments(binary: &Path) -> Result<()> {
    ...
    let new = completed(
        &root.control(),
        json!({"id":1,"command":"new","instance":instance,"argv":argv}),
    )?["pane"]
        .clone();
    ...
    for bad in [json!([""]), json!(["/bin/sh", "\0"])] {
        let reply = rpc(
            &root.control(),
            json!({"id":1,"command":"new","instance":instance,"argv":bad}),
        )?;
        ensure!(
    ...
    server.finish()?;
    println!("empty config/new/split arguments and retained final argv passed");
    Ok(())
}
