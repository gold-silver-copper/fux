//! Main rejects incompatible managers without replacing or stopping their sessions.
use crate::support::{
...
pub(super) fn run(binary: &Path) -> Result<()> {
    ...
    let descriptor_path = root.path().join("fux/workspaces/default.json");
    ...
    let scenario = (|| -> Result<()> {
        ...
        let stderr = String::from_utf8_lossy(&result.stderr);
            ...
            !result.status.success()
                && stderr.contains("cannot use the existing session server")
                && stderr.contains("save your work")
                && stderr.contains("not a fux control socket"),
            "missing noninteractive rejection: {stderr}"
        );
        ensure!(
            ...
            fs::read(&descriptor_path)? == descriptor,
            "rejection changed old descriptor"
        );
        {
            let mut terminal = Terminal::start(&root, binary)?;
            ensure!(
                !terminal.wait(Duration::from_secs(20))?.success(),
                "interactive incompatible attach succeeded"
            );
            let output = terminal.take_output();
            ensure!(
                String::from_utf8_lossy(&output).contains("save your work"),
                "missing interactive rejection"
            );
            ensure!(
                !output
                    .windows(b"\x1b[?1049h".len())
                    .any(|bytes| bytes == b"\x1b[?1049h"),
                "rejected attach entered alternate screen"
            );
            ensure!(
                old.0.try_wait()?.is_none(),
                "interactive fux stopped old manager"
            );
            ensure!(
                fs::read(&descriptor_path)? == descriptor,
                "interactive rejection changed descriptor"
            );
        }
        // The fixture explicitly performs the operator's shutdown; production has no migration dialog.
        nix::sys::signal::kill(
            nix::unistd::Pid::from_raw(i32::try_from(old.0.id())?),
            nix::sys::signal::Signal::SIGTERM,
        )?;
        ensure!(
            process::wait(&mut old.0, Duration::from_secs(5))?.success(),
            "owned old manager shutdown"
        );
    ...
    println!(
        "PASS incompatible manager rejection preserves sessions; explicit owned shutdown allows fresh startup"
    );
    Ok(())
}
