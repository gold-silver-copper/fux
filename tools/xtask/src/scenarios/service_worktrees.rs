...
pub fn run(h: &Harness<'_>) -> Result<Managed> {
    ...
    let pane = h.fux("new", json!({"cwd":tree_path,"argv":["/bin/cat"]}))?["pane"].clone();
    ...
    fs::remove_file(&marker)?;
    let pane = h.fux("new", json!({"cwd":root,"argv":moved_argv}))?["pane"].clone();
    marked(&marker, path(&tree_path)?)?;
    let before = fs::read(&h.journal)?;
    refuse(
        h,
    ...
    let pane=h.fux("new",json!({"cwd":root,"argv":[std::env::current_exe()?,"fixture-worker","service-descendant",tree_path,descendant,"1"]}))?["pane"].clone();
