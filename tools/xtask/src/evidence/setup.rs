...
fn report_checks(value: &Value, historical: bool) -> Result<()> {
    ...
    if historical {
        retained_sources("controller-setup", value, "/provenance/source_hashes")?;
        let original = retained_report("controller-setup")?;
        for key in ["harness_kind", "harness_sha256", "worker_sha256"] {
            ensure!(
                value["provenance"][key] == original["provenance"][key],
                "historical setup {key} changed"
            );
        }
    } else {
    ...
    let build = if historical {
        ...
        field(value, "/provenance/herdr_build/binary_sha256")?
            ...
            && field(value, "/provenance/herdr_build")? == &build,
        "build provenance"
    );
...
pub fn regressions() -> Result<()> {
    let value = retained_report("controller-setup")?;
...
mod tests {
    #[test]
    fn historical_setup_cannot_validate_current_sources() -> anyhow::Result<()> {
        use super::*;
        let mut value = retained_report("controller-setup")?;
        report_checks(&value, true)?;
        ensure!(
            report_checks(&value, false).is_err(),
            "old setup accepted as current evidence"
        );
        value["provenance"]["worker_sha256"] = json!("0".repeat(64));
        ensure!(
            report_checks(&value, true).is_err(),
            "wrong archived worker accepted"
        );
        Ok(())
    }
