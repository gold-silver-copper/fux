...
pub fn regressions() -> Result<()> {
    let value = retained_report("resources")?;
    retained_sources("resources", &value, "/provenance/sources")?;
    ensure!(
...
mod tests {
    #[test]
    fn retained_sources_reject_changed_or_missing_entries() -> anyhow::Result<()> {
        use super::*;
        let mut value = retained_report("resources")?;
        value["provenance"]["sources"]["src/ecs/systems/output.rs"] = json!("0".repeat(64));
        ensure!(
            retained_sources("resources", &value, "/provenance/sources").is_err(),
            "wrong historical hash accepted"
        );
        value["provenance"]["sources"]
            .as_object_mut()
            .context("sources")?
            .remove("src/ecs/systems/output.rs");
        ensure!(
            retained_sources("resources", &value, "/provenance/sources").is_err(),
            "missing historical source accepted"
        );
        Ok(())
    }
    #[test]
