...
fn validate(value: &Value) -> Result<()> {
    ...
    for (case, row) in cases.iter().zip(rows) {
        ...
        field(value, "/provenance/binaries/herdr/sha256")?
            == field(&report("controller-setup-build")?, "/binary_sha256")?,
        "reference build mismatch"
    );
    Ok(())
}
pub fn regressions() -> Result<()> {
    let value = report("detection-screens")?;
    validate(&value)?;
    ...
    missing["results"]
        ...
        .context("zor verdict")?
        .remove("rule");
    ensure!(validate(&missing).is_err(), "missing null rule accepted");
    let mut invalid = value.clone();
    invalid["results"].as_array_mut().context("results")?.pop();
    ensure!(validate(&invalid).is_err(), "omitted negative accepted");
    let mut invalid = value.clone();
    invalid["summary"]["zor"]["correct"] = (integer(&value, "/summary/zor/correct")? + 1).into();
    ensure!(validate(&invalid).is_err(), "invented score accepted");
    let mut invalid = value.clone();
    invalid["results"]
        .as_array_mut()
        .context("results")?
...
pub fn verify(args: Vec<String>) -> Result<()> {
    match args.as_slice() {
        [] => regressions(),
        [path] => validate(&serde_json::from_slice(&fs::read(path)?)?),
