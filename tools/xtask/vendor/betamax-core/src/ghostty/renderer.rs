...
impl RasterRenderer {
    pub(super) fn cell_size(&self) -> (u32, u32) {
        (self.cell_width, self.cell_height)
    }

    pub(super) fn resize(
        &mut self,
        grid: super::engine::TerminalGrid,
        canvas: super::engine::PixelSize,
    ) {
        self.request.grid = grid;
        self.request.canvas = canvas;
    }
    ...
    pub(super) fn render(
        ...
        // Paint every background before any glyph: a following spacer cell must not
        // erase the second half of a wide glyph (or a preceding glyph's overhang).
            ...
            while let Some(row) = row_iter.next() {
                ...
                while let Some(cell) = cell_iter.next() {
                    ...
                    if !style.invisible && !graphemes.is_empty() {
                        ...
                        glyphs.push((text, x_px, y_px, foreground, style));
                    }
        ...
        for (text, x, y, foreground, style) in glyphs {
            if draw_box_glyph(
                &mut target,
                &text,
                x,
            ...
            self.text_renderer
                .draw_text(&mut target, &text, x, y, foreground, style);
        }
        ...
        while let Some(row) = row_iter.next() {
            ...
            while let Some(cell) = cell_iter.next() {
                if cell
                    ...
                    .map_err(vt_error("failed to read cell width"))?
...
/// Terminal box-drawing strokes occupy the cell, independent of font fallback
/// metrics and line spacing. Keep junctions aligned when focus changes weight.
...
fn draw_box_glyph(
    ...
    let stroke = if bold { 2 } else { 1 }.min(width).min(height).max(1);
    let cx = width / 2;
    let cy = height / 2;
    let left = cx.saturating_sub(stroke / 2);
    let top = cy.saturating_sub(stroke / 2);
    if arms & 1 != 0 {
        target.fill_rect(x, y + top, left + stroke, stroke, color);
    }
    if arms & 2 != 0 {
        target.fill_rect(x + left, y + top, width - left, stroke, color);
    }
    if arms & 4 != 0 {
        target.fill_rect(x + left, y, stroke, top + stroke, color);
    }
    if arms & 8 != 0 {
        target.fill_rect(x + left, y + top, stroke, height - top, color);
    }
    true
}
