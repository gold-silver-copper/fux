...
impl GhosttySession {
    /// Whether the application is inside a DEC synchronized-output frame.
    ///
    /// # Errors
    /// Returns an error if libghostty-vt cannot read the mode.
    pub fn synchronized_output(&self) -> Result<bool> {
        Ok(self
            .terminal
            .mode(libghostty_vt::terminal::Mode::SYNC_OUTPUT)
            .map_err(vt_error("failed to read synchronized-output mode"))?)
    }

    /// Resize a live terminal and raster canvas while preserving both screen buffers.
    ///
    /// # Errors
    /// Returns an error when libghostty-vt cannot resize its terminal state.
    pub fn resize(&mut self, grid: TerminalGrid, canvas: PixelSize) -> Result<()> {
        ...
        self.renderer.resize(grid, canvas);
        Ok(())
    }
