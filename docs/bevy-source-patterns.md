...
- `Drag`, `DragEnd` and `DragDrop { dropped }` keep naming the entity that was *pressed* for the whole drag (`events.rs:1004-1031`, `1111-1127`) even after it is despawned: `commands.trigger` on a dead entity still runs global observers (`bevy_ecs/src/event/trigger.rs:283-312` only stops the *propagation* loop). fux's border drag re-clones instances on every live patch, so drag state lives on the viewer's pointer entity (`crates/fux/src/pointer.rs`, `PointerDrag`), never on the dragged instance; the `Drag` observer ignores `event.entity`'s liveness. `DragStart` carries the press `hit` (`events.rs:1073-1081`), `Drag` does not — read `HitData::extra` there and remember it.
- Global observers of a propagating `Pointer<E>` run once per hop with `event.entity` set to the current ancestor; gate one-shot policy on `event.entity == event.original_event_target()` (`bevy_ecs/src/observer/system_param.rs:139`) rather than on `propagate(false)`, which would also hide the event from the ancestors.
...
**Caveat (found by the border-drag implementation).** With `flex_basis: 0px` a flex item's borders and padding still count: taffy's automatic minimum size and the border-box basis leave `size = insets + free_space * grow / Σgrow` (`convert.rs:104-105` copies `flex_grow` verbatim; the inset handling is taffy's). Writing `flex_grow = target_cells` therefore overshoots by the insets; fux writes `flex_grow = target_cells - content_inset` for **every** in-flow sibling of the container (`crates/fux/src/pointer.rs`, `patch_grow`), which makes the free space exactly the sum of the grows and lays each item out at its target.
...
### src/io/file/file_watcher.rs, src/io/source.rs (hot reload under an idle runner)

**(a) Mechanism.** The default source (`source.rs:303-323`, `platform_default`) is `FileAssetReader` + `FileAssetWriter` + `FileWatcher` (notify-debouncer-full, 300 ms, `RecursiveMode::Recursive` on the root, `file_watcher.rs:82-250`). Watcher events go into an `async_channel` and are turned into `reload_internal` calls only inside `handle_internal_asset_events` (`server/mod.rs:2110-2210`), which is a `PreUpdate` system; a reload's result is likewise queued and applied by the next `handle_internal_asset_events`. `AssetSourceBuilder::with_reader`/`with_watcher` (`source.rs:229-256`) replace either half with any `ErasedAssetReader`/`AssetWatcher` factory; inserting the builder under `AssetSourceId::Default` before `AssetPlugin` wins because `init_default_source` is `get_or_insert_with` (`source.rs:382-385`).

**(b) fux use (3.7).** `assets::waking_source` keeps bevy's reader and watcher and only adds a wake: a forwarding task between the watcher and bevy's channel calls the runner's wake after every event, and the `Reader` handed to the loader wakes it on drop, which the server reaches after it queued `Loaded`/`Failed` (`server/mod.rs:870-948`). Without both, a runner that sleeps in `block_on` until an `Inbound` arrives (3.1, zero idle wake-ups) applies an edited `fux.toml` only when unrelated traffic happens to step the App. The viewer runner gets the same treatment through `Wake::Asset`.

**(c) Pitfalls.** (1) `get_asset_path` panics the watcher thread when an event path cannot be stripped of the root (`file_watcher.rs:62-66`); notify reports canonical paths, so a root under a symlink (`/var` → `/private/var` on macOS, a symlinked `$XDG_CONFIG_HOME`) silently ends hot reload — fux canonicalises the root (`assets::asset_root`) and creates the config directory before `AssetPlugin` because a missing root skips the watcher altogether (`source.rs:553-567`). (2) A failed reload does not remove the previous asset from `Assets<A>` (`info.rs:637-679` only flips the load state), which is exactly the "keep the previous asset" rule of 3.7, but `reload_internal` retries the path untyped after the typed reload fails (`server/mod.rs:988-994`), so one bad edit produces two `error!` lines and two `AssetLoadFailedEvent`s: report the last one per update, not each. (3) A missing file at startup is a `NotFound` failure with bevy's own `error!`; holding the failed handle is what makes a later creation reload, so the load must not be skipped just because the file is absent.
