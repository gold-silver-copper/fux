...
with tempfile.TemporaryDirectory(prefix='zmr-', dir='/tmp') as directory:
    ...
    try:
        ...
        for machine, title in [('remote','first task'),('second','second task')]:
            ...
            task = inspected['value']
            expected = dict(task='same',attempt=task['task']['attempt'],session=task['attempt']['session'],target=task['session']['target'])
            request = dict(v=1,id=73,op='task',service_instance=inspected['service_instance'],task=dict(action='attachment-target',expected=expected))
            remote_root = remote_roots[0 if machine == 'remote' else 1]
            with socket.socket(socket.AF_UNIX,socket.SOCK_STREAM) as stream:
                stream.settimeout(3)
                stream.connect(str(remote_root/'zor/control.sock'))
                stream.sendall(json.dumps(request).encode()+b'\n')
                resolved = json.loads(stream.makefile('rb').readline(524289))
            assert resolved['status'] == 'completed', resolved
            assert resolved['value']['pid'] == task['session']['target']['pid']
            assert resolved['value']['instance'] == task['session']['target']['instance']
            assert resolved['value']['workspace'] == 'default'
        ...
        assert all(child.poll() is None for child in children)
        print(json.dumps({'result':'passed', 'checks':['guarded live attachment route resolution','same-named task list/inspect/results','guarded cancel/managed stop isolation, explicit reconciliation and adopted stop refusal','saved ID routing','snapshot/view incarnation',
