...
manifest = {'started':now(),'complete':False,'runs':[],'orchestrator_sha256':digest(pathlib.Path(__file__)), 'harness':{'path':str(harness),'sha256':digest(harness)},
    ...
    'limits':'Local synthetic workloads; discrete resource observations are not kernel peak or physical disk writes. No release build or Betamax capture overlaps measurement.'}
def save():
...
for block in range(3):
    for workload in ['idle-burst','many-viewers','resize','history-memory','scroll-manager','recovery','journal','pressure']:
