...
followup=json.loads((work/'release-followup-diagnostic-20260913/manifest.json').read_text())
...
for variant in ['baseline','candidate']:
 ...
 for binary in ['fux','zor']:
  ...
  (work/f'provenance-{variant}-{binary}.jsonl').write_text(result.stdout)
  (work/f'provenance-{variant}-{binary}.log').write_text(result.stderr)
