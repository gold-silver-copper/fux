...
assert manifest['complete'] and len(manifest['runs'])==6 and all(r['complete'] and r['exit_code']==0 for r in manifest['runs'])
...
for run in manifest['runs']:
 ...
 if w=='idle-burst':
  ...
  for name,key in [('recovery latency (ms)','recovery_ms'),('recovery retry latency (ms)','idempotent_recovery_ms'),('recovery server CPU (s/op)','server_cpu_s'),('recovery child CPU (ms/op)','recovery_child_cpu_ms'),('recovery retry child CPU (ms/op)','retry_child_cpu_ms')]:put(name,p,b,[r[key] for r in x['samples']])
  traces=[r['retry_proxy_timings'] for r in x['samples']]
  for sample,trace in zip(x['samples'],traces):
   assert [t['operation'] for t in trace]==['pane-location','release-pane-pin'],trace
   assert all(0<=t['accepted_ms']<=t['requested_ms']<=t['finished_ms']<=sample['idempotent_recovery_ms'] for t in trace),trace
   assert trace[0]['finished_ms']<=trace[1]['accepted_ms'],trace
  put('retry first accept since CLI start (ms)',p,b,[t[0]['accepted_ms'] for t in traces])
  put('retry proxy handling sum (ms)',p,b,[sum(q['finished_ms']-q['accepted_ms'] for q in t) for t in traces])
  put('retry between proxy requests (ms)',p,b,[t[1]['accepted_ms']-t[0]['finished_ms'] for t in traces])
  put('retry after last proxy response (ms)',p,b,[r['idempotent_recovery_ms']-t[-1]['finished_ms'] for r,t in zip(x['samples'],traces)])
 elif w=='journal':
